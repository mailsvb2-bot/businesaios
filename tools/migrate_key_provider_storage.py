...
class KeyProviderStorageMigrationPlan:
    vault_path: Path
...
def _parse_datetime(value: object, *, field_name: str) -> datetime:
    ...
    if not text:
        raise RuntimeError(f"inline key field {field_name} is required")
    parsed = datetime.fromisoformat(text.replace("Z", "+00:00"))
    if parsed.tzinfo is None:
        raise RuntimeError(f"inline key field {field_name} must be timezone-aware")
    return parsed
...
def _inline_key_record(payload: Mapping[str, object]) -> KeyMaterialRecord:
    key_id = str(payload.get("key_id") or "").strip()
    encoded = str(payload.get("secret_b64") or "").strip()
    if not encoded:
        raise RuntimeError(f"inline key {key_id or '<unknown>'} has no secret_b64")
    try:
        secret = base64.b64decode(encoded, validate=True)
    except Exception as exc:
        raise RuntimeError(f"inline key {key_id or '<unknown>'} has invalid secret_b64") from exc
    record = KeyMaterialRecord(
        ...
        metadata={**dict(payload.get("metadata") or {}), "migrated_from": "inline_secret_vault_v1"},
    )
    record.validate()
    return record


def build_migration_plan(*, vault_path: str | Path, key_provider_path: str | Path) -> KeyProviderStorageMigrationPlan:
    ...
    if not source.is_file():
        raise RuntimeError(f"source vault file does not exist: {source}")
    try:
        payload = json.loads(source.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise RuntimeError(f"source vault cannot be read: {source}") from exc
    if not isinstance(payload, dict):
        raise RuntimeError("inline-key vault must be a JSON object")
    raw_keys = payload.get("keys")
    if not isinstance(raw_keys, list) or not raw_keys:
        raise RuntimeError("vault contains no inline keys to migrate")
    records = tuple(_inline_key_record(dict(item)) for item in raw_keys if isinstance(item, Mapping))
    if len(records) != len(raw_keys):
        raise RuntimeError("every inline key entry must be an object")
    key_ids = [record.key_id for record in records]
    if len(key_ids) != len(set(key_ids)):
        raise RuntimeError("vault contains duplicate key_id values")
    backup = source.with_suffix(source.suffix + ".inline-keys-v1.bak")
    return KeyProviderStorageMigrationPlan(
        vault_path=source,
...
def apply_migration(plan: KeyProviderStorageMigrationPlan) -> dict[str, object]:
    ...
    migrated_payload = {key: value for key, value in dict(plan.vault_payload).items() if key != "keys"}
    migrated_payload["key_storage"] = "external_key_provider"
    migrated_payload["migration"] = {
        "source": "inline_secret_vault_v1",
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Move inline secret-vault keys into the authenticated external FileKeyProvider.",
    )
    parser.add_argument("--vault-path", required=True, help="Path to the source secret_vault.json")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    plan = build_migration_plan(vault_path=args.vault_path, key_provider_path=args.key_provider_path)
