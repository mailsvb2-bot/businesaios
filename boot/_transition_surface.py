...
def validate_transition_exports(*, module_label: str, exports: Mapping[str, str]) -> None:
    ...
    for export_name, module_name in dict(exports).items():
        ...
        if normalized_module in seen_modules:
            raise ValueError(
                f"{module_label} transition alias '{normalized_module}' is declared more than once"
            )
        seen_modules.add(normalized_module)


def install_boot_transition_aliases(
    *,
    package_name: str,
    owner_module: str,
    ...
    validate_transition_exports(module_label=module_label, exports=exports)
...
__all__ = [
    "install_boot_transition_aliases",
    "validate_transition_exports",
]
