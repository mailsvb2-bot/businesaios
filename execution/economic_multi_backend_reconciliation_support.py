...
def build_reconciliation_fields(*, feedback_rows, roi_rows, snapshot_rows, trace_rows, metrics_rows, bundle_payloads=(), node_payloads=(), quorum_size=None, segment_quorum_policy=None, backend_role_policy=None) -> dict[str, Any]:
    ...
    consistent = not any((missing_feedback, missing_roi, missing_snapshots, missing_traces, missing_metrics, tuple(inconsistent_nodes), tuple(scope_mismatch_nodes), tuple(profile_mismatch_nodes), tuple(corrupted_nodes), tuple(quorum_failure_segments), stale_node_ids, tuple(distributed_extremes.get('issues') or ())))
    ...
    return {
        ...
        'metadata': {
            ...
            'local_counts': {'feedback': len(local_feedback), 'roi': len(local_roi), 'snapshots': len(local_snapshots), 'traces': len(local_traces), 'metrics': len(local_metrics)},
            'bundle_counts': {'feedback': len(bundle_feedback), 'roi': len(bundle_roi), 'snapshots': len(bundle_snapshots), 'traces': len(bundle_traces), 'metrics': len(bundle_metrics)},
