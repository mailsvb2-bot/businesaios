...
class DunningPolicy:
    ...
    def validate(self) -> None:
        require_commercial_int("grace_days", self.grace_days, minimum=0)
        if not self.retry_delays_days:
            raise ValueError("retry_delays_days must not be empty")
        for delay in self.retry_delays_days:
            require_commercial_int("retry delay", delay, minimum=0)
