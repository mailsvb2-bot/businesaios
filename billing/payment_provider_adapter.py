...
class RoutingPaymentProviderAdapter(PaymentProviderContract):
    ...
    def ensure_customer(
        ...
        *,
        ...
        be followed by the next healthy candidate without duplicating a charge or
        refund. Monetary operations remain single-attempt after dispatch.
        """
        ...
        for provider in providers:
            ...
            try:
                ...
                if normalized.default_currency != currency:
                    raise ValueError(brouted provider returned mismatched default_currency")
    ...
    def _customer_provisioning_key(*, tenant_id: str, email: str | None, currency: str) -> str:
        material = "|".join((str(tenant_id).strip(), str(email or "").strip().lower(), str(currency).strip().upper()))
