...
class PaymentCollectionOrchestrator:
    ...
    def collect(self, *, invoice: CommercialInvoiceEnvelope, idempotency_key: str, attempt_no: int = 1, metadata: Mapping[str, object] | None = None) -> tuple[CommercialInvoiceEnvelope, CommercialCollectionResult]:
        invoice.validate()
        normalized_key = str(idempotency_key or '').strip()
        if not normalized_key:
            raise ValueError('idempotency_key is required')
        existing = self._result_store.get_by_idempotency(tenant_id=invoice.tenant_id, invoice_id=invoice.invoice_id, idempotency_key=normalized_key)
        if existing is not None:
            ...
            if str(existing_metadata.get('currency', invoice.currency)).strip().upper() != str(invoice.currency).strip().upper():
                raise ValueError('replayed collection currency mismatch')
            replayed_invoice = invoice
            replay_amount_minor = int(existing_metadata.get('collected_amount_minor', 0) or 0)
            if existing.successful and replay_amount_minor > 0 and invoice.remaining_minor > 0:
                replayed_invoice = self._invoice_lifecycle.record_payment(invoice, amount_minor=min(invoice.remaining_minor, replay_amount_minor))
        ...
        if amount_minor <= 0:
            synthetic = CommercialCollectionResult(
                ...
                metadata={'owner': 'billing.payment_collection', 'noop': True, 'idempotency_key': normalized_key, 'currency': invoice.currency, 'collected_amount_minor': 0},
            )
            saved = self._result_store.append(synthetic, idempotency_key=normalized_key)
            if self._metrics is not None:
                self._metrics.inc(tenant_id=invoice.tenant_id, metric_name='billing_collection_attempts_total', amount=1.0, labels={'provider': saved.provider_name, 'successful': 'true', 'noop': 'true'})
        ...
        if provider_result.successful and provider_result.retryable:
            raise ValueError('successful collection result cannot be retryable')
        result = replace(
            provider_result,
            metadata={
        ...
        if self._metrics is not None:
            self._metrics.inc(tenant_id=invoice.tenant_id, metric_name='billing_collection_attempts_total', amount=1.0, labels={'provider': saved_result.provider_name, 'successful': str(saved_result.successful).lower()})
        return updated_invoice, saved_result

    def _validate_provider_result(self, *, attempt: CommercialCollectionAttempt, result: CommercialCollectionResult) -> None:
        result.validate()
