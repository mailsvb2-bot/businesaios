...
class SqliteRefundStore:
    ...
    def _init_db(self) -> None:
        with self._connect() as conn:
            ...
            conn.execute(
                'CREATE UNIQUE INDEX IF NOT EXISTS idx_billing_refund_idem ON billing_refund_results(tenant_id, invoice_id, idempotency_key) WHERE idempotency_key IS NOT NULL'
            )
    ...
    def save(self, result: 'RefundResult', *, idempotency_key: str | None = None) -> 'RefundResult':
        ...
        try:
            with self._connect() as conn:
                row = conn.execute(
                    'SELECT payload_json FROM billing_refund_results WHERE tenant_id = ? AND refund_id = ?',
                    (tenant_id, refund_id),
                ).fetchone()
                if row is not None:
                ...
                conn.execute(
                    '''
                    INSERT INTO billing_refund_results(tenant_id, invoice_id, refund_id, idempotency_key, processed_at, payload_json)
                    VALUES (?, ?, ?, ?, ?, ?)
                    ''',
    ...
    def get_by_idempotency(self, *, tenant_id: str, invoice_id: str, idempotency_key: str) -> 'RefundResult | None':
        ...
        with self._connect() as conn:
            row = conn.execute(
                'SELECT payload_json FROM billing_refund_results WHERE tenant_id = ? AND invoice_id = ? AND idempotency_key = ?',
                (tid, iid, idem),
            ).fetchone()
    ...
    def list_for_invoice(self, *, tenant_id: str, invoice_id: str) -> tuple['RefundResult', ...]:
        ...
        with self._connect() as conn:
            rows = conn.execute(
                'SELECT payload_json FROM billing_refund_results WHERE tenant_id = ? AND invoice_id = ? ORDER BY processed_at ASC, refund_id ASC',
                (tid, iid),
            ).fetchall()
        return tuple(self._decode_result_payload(row[0]) for row in rows)

    def _decode_result_payload(self, payload_json: str) -> 'RefundResult':
        from billing.refund_orchestrator import RefundResult

        payload = json.loads(payload_json)
        payload['processed_at'] = datetime.fromisoformat(payload['processed_at'])
        result = RefundResult(**payload)
        result.validate()
        return result


class SqliteChargebackStore:
    def __init__(self, *, sqlite_path: str) -> None:
        self._path = str(sqlite_path).strip()
        if not self._path:
            raise ValueError('sqlite_path is required')
    ...
    def _init_db(self) -> None:
        with self._connect() as conn:
            ...
            conn.execute(
                'CREATE UNIQUE INDEX IF NOT EXISTS idx_billing_chargeback_idem ON billing_chargeback_cases(tenant_id, invoice_id, idempotency_key) WHERE idempotency_key IS NOT NULL'
            )
    ...
    def save(self, case: 'ChargebackCase', *, idempotency_key: str | None = None) -> 'ChargebackCase':
        ...
        try:
            with self._connect() as conn:
                row = conn.execute(
                    'SELECT payload_json FROM billing_chargeback_cases WHERE tenant_id = ? AND case_id = ?',
                    (tenant_id, case_id),
                ).fetchone()
                if row is not None:
                ...
                conn.execute(
                    '''
                    INSERT INTO billing_chargeback_cases(tenant_id, invoice_id, case_id, idempotency_key, opened_at, payload_json)
                    VALUES (?, ?, ?, ?, ?, ?)
                    ''',
    ...
    def get_by_idempotency(self, *, tenant_id: str, invoice_id: str, idempotency_key: str) -> 'ChargebackCase | None':
        ...
        with self._connect() as conn:
            row = conn.execute(
                'SELECT payload_json FROM billing_chargeback_cases WHERE tenant_id = ? AND invoice_id = ? AND idempotency_key = ?',
                (tid, iid, idem),
            ).fetchone()
    ...
    def list_for_invoice(self, *, tenant_id: str, invoice_id: str) -> tuple['ChargebackCase', ...]:
        ...
        with self._connect() as conn:
            rows = conn.execute(
                'SELECT payload_json FROM billing_chargeback_cases WHERE tenant_id = ? AND invoice_id = ? ORDER BY opened_at ASC, case_id ASC',
                (tid, iid),
            ).fetchall()
        return tuple(self._decode_case_payload(row[0]) for row in rows)

    def _decode_case_payload(self, payload_json: str) -> 'ChargebackCase':
        from billing.chargeback_orchestrator import ChargebackCase

        payload = json.loads(payload_json)
        payload['opened_at'] = datetime.fromisoformat(payload['opened_at'])
        case = ChargebackCase(**payload)
        case.validate()
        return case
