...
class SqliteCollectionResultStore:
...
class SqliteLedgerStore(LedgerStoreContract):
    def __init__(self, *, sqlite_path: str) -> None:
        self._path = str(sqlite_path)
        if not self._path.strip():
            raise ValueError('sqlite_path is required')
...
__all__ = ['CANON_BILLING_SQLITE_STORE', 'SqliteCollectionResultStore', 'SqliteLedgerStore']
