...
class SubscriptionLifecycleService:
    ...
    def activate(
        ...
        *,
        ...
        require_aware_datetime("activated_at", started_at)
        normalized_trial_days = require_commercial_int(
            "trial_days",
            trial_days,
            minimum=0,
        )
        cycle = next_cycle_window(current_start_at=started_at, interval=interval)
        status = SubscriptionLifecycleStatus.TRIALING if normalized_trial_days else SubscriptionLifecycleStatus.ACTIVE
        trial_ends_at = started_at + timedelta(days=normalized_trial_days) if normalized_trial_days else None
    ...
    def mark_past_due(
        ...
        *,
        ...
        observed_at = now or utc_now()
        require_aware_datetime("now", observed_at)
        normalized_grace_days = require_commercial_int(
            "grace_days",
            grace_days,
            minimum=0,
        )
        ...
        updated = replace(
            ...
            grace_until=observed_at + timedelta(days=normalized_grace_days),
        )
        updated.validate()
        return updated
    ...
    def enter_grace(
        ...
        *,
        ...
        updated = replace(
            ...
            grace_until=envelope.grace_until or (observed_at + timedelta(days=3)),
        )
        updated.validate()
        return updated

    def suspend_if_expired(
        self,
        envelope: SubscriptionCommercialEnvelope,
        *,
        now: datetime | None = None,
    ...
    def renew_cycle(
        ...
        *,
        ...
        next_cycle = next_cycle_window(
            current_start_at=cycle_start_at,
            interval=anchor,
        )
        updated = replace(
            envelope,
    ...
    def cancel(
        ...
        *,
        ...
        require_aware_datetime("canceled_at", when)
        updated = replace(
            envelope,
    ...
    def plan_change_proration_fraction(
        self,
        *,
        ...
        cycle.validate()
        require_aware_datetime("changed_at", changed_at)
