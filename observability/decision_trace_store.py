...
from observability.observability_store_paths import decision_trace_path
...
CANON_DECISION_TRACE_STORE = True



def decision_trace_store_path(*, config_surface=None) -> Path:
    return decision_trace_path(config_surface=config_surface)
...
class InMemoryDecisionTraceStore:
    ...
    def get(self, *, tenant_id: str, decision_id: str) -> DecisionTraceEvent | None:
        ...
        for row in reversed(self._rows):
            if row['tenant_id'] == tid and str(row['decision_id']) == needle:
    ...
    def validate_chain(self) -> None:
        ...
        for row in self._rows:
            if row.get('previous_hash') != previous_hash:
                raise ValueError('decision trace chain previous_hash mismatch')
            expected_hash = self._record_hash({k: v for k, v in row.items() if k != 'record_hash'})
            if row.get('record_hash') != expected_hash:
                raise ValueError('decision trace chain record_hash mismatch')
            previous_hash = expected_hash

    @staticmethod
    def _canonical_payload(event: DecisionTraceEvent) -> dict[str, Any]:
    ...
    @staticmethod
    def _deserialize(payload: dict[str, Any]) -> DecisionTraceEvent:
        ...
        return DecisionTraceEvent(
            ...
            candidate_count=None if payload.get('candidate_count') is None else int(payload['candidate_count']),
...
class PersistentDecisionTraceStore:
    def __init__(self, path: str | Path | None = None, *, storage_policy: TraceStoragePolicy | None = None, config_surface: BootstrapConfigSurface | None = None) -> None:
        self._path = Path(path) if path is not None else decision_trace_store_path(config_surface=config_surface)
    ...
    def get(self, *, tenant_id: str, decision_id: str) -> DecisionTraceEvent | None:
        ...
        for row in reversed(rows):
            if row.get('tenant_id') == tid and str(row.get('decision_id') or '') == needle:
                return self._deserialize(row)
        return None
