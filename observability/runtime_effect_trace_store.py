...
from observability.execution_trace_contract import EffectDisposition, RuntimeEffectTraceEvent
from observability.storage_coordination import advisory_file_lock
from observability.observability_store_paths import runtime_effect_trace_path
...
CANON_RUNTIME_EFFECT_TRACE_STORE = True



def runtime_effect_trace_store_path(*, config_surface=None) -> Path:
    return runtime_effect_trace_path(config_surface=config_surface)
...
class NullRuntimeEffectTraceStore:
...
class InMemoryRuntimeEffectTraceStore:
    ...
    def list_by_trace(self, *, tenant_id: str, trace_id: str) -> tuple[RuntimeEffectTraceEvent, ...]:
        ...
        items.sort(key=lambda x: x.emitted_at)
        return tuple(items)
    ...
    def list_by_effect_type(self, *, tenant_id: str, effect_type: str, limit: int = 200) -> tuple[RuntimeEffectTraceEvent, ...]:
        ...
        items = [
            ...
            if row['tenant_id'] == tid and row['effect_type'] == effect_type
        ]
    ...
    def validate_chain(self) -> None:
        ...
        for row in self._rows:
            if row.get('previous_hash') != previous_hash:
                raise ValueError('runtime effect trace chain previous_hash mismatch')
            expected_hash = self._record_hash({k: v for k, v in row.items() if k != 'record_hash'})
            if row.get('record_hash') != expected_hash:
                raise ValueError('runtime effect trace chain record_hash mismatch')
            previous_hash = expected_hash

    @staticmethod
    def _canonical_payload(event: RuntimeEffectTraceEvent) -> dict[str, Any]:
    ...
    @staticmethod
    def _deserialize(payload: dict[str, Any]) -> RuntimeEffectTraceEvent:
        return RuntimeEffectTraceEvent(
            tenant_id=str(payload['tenant_id']),
            trace_id=str(payload['trace_id']),
            effect_id=str(payload['effect_id']),
            effect_type=str(payload['effect_type']),
            disposition=EffectDisposition(str(payload['disposition'])),
...
class PersistentRuntimeEffectTraceStore:
    def __init__(self, path: str | Path | None = None, *, storage_policy: TraceStoragePolicy | None = None, config_surface: BootstrapConfigSurface | None = None) -> None:
        self._path = Path(path) if path is not None else runtime_effect_trace_store_path(config_surface=config_surface)
    ...
    def list_by_effect_type(self, *, tenant_id: str, effect_type: str, limit: int = 200) -> tuple[RuntimeEffectTraceEvent, ...]:
        ...
        items = [
            self._deserialize(row)
            for row in reversed(self._read_all_segments())
            if row.get('tenant_id') == tid and row.get('effect_type') == effect_type
        ]
        return tuple(items[: max(0, int(limit))])
