...
CANON_POSTGRES_DECISION_ARCHIVE = True
...
class PostgresDecisionArchive:
    def __init__(self, dsn: str, *, tenant_id: str | None = None):
        ...
        self._session_factory = PostgresSessionFactory(
            self._dsn,
            application_name="businesaios-archive",
    ...
    def get(self, decision_id: str) -> Optional[Any]:
        row = self._db.fetchone(
            "SELECT envelope_json FROM decision_archive WHERE decision_id=%s LIMIT 1;",
            (str(decision_id),),
        )
        if not row:
            return None
        return _rebuild_envelope(json.loads(str(row.get("envelope_json") or "{}")))
