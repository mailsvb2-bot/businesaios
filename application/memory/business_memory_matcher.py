...
class BusinessMemoryMatcher:
    ...
    def _score(self, *, target: MemoryRunFingerprint, candidate: MemoryRunFingerprint) -> float:
        score=0.0
        if target.goal_family and target.goal_family == candidate.goal_family:
            score += 0.40
        if target.channel and target.channel == candidate.channel:
            score += 0.20
        if target.region and target.region == candidate.region:
            score += 0.10
        if target.segment and target.segment == candidate.segment:
            score += 0.10
        if target.offer_type and target.offer_type == candidate.offer_type:
            score += 0.10
        if target.traffic_source and target.traffic_source == candidate.traffic_source:
            score += 0.10
        return float(score)
    def goal_family(self, goal: str) -> str:
        text=_norm(goal)
        if any(token in text for token in ("acquire", "lead", "funnel", "traffic", "client")):
            return "acquisition"
        if any(token in text for token in ("retain", "churn", "repeat", "loyal")):
            return "retention"
        if any(token in text for token in ("monet", "revenue", "price", "ltv", "cac")):
            return "monetization"
        if any(token in text for token in ("ops", "stabil", "latency", "quality", "execute")):
            return "operations"
        return "general"
