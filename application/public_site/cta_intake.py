...
def _status_from_row(*, token: str, row: dict[str, object]) -> CTAIntakeStatus:
    ...
    if isinstance(selected_raw, list):
        selected_providers = tuple(str(item) for item in selected_raw if str(item).strip())
    else:
        selected_providers = _selected_providers(payload)

    integration_raw = row.get("integration_plan")
    if isinstance(integration_raw, list):
        integration_plan = tuple(item for item in integration_raw if isinstance(item, dict))
    else:
        integration_plan = _integration_plan(selected_providers)
    ...
    business_profile = row.get("business_profile") if isinstance(row.get("business_profile"), dict) else _business_profile(payload)
    autonomy_mode = str(row.get("autonomy_mode") or _autonomy_mode(payload))

    first_value_preview = row.get("first_value_preview") if isinstance(row.get("first_value_preview"), dict) else None
    if first_value_preview is None:
        first_value_preview = _first_value_preview(
            business_profile=dict(business_profile),
            selected_providers=selected_providers,
            goal=str(business_profile.get("goal") or "growth"),
        )
    ...
    return CTAIntakeStatus(
        ...
        integration_plan=integration_plan,
        autonomy_mode=autonomy_mode,
        first_value_preview=dict(first_value_preview),
        onboarding_progress=dict(onboarding_progress),
    )


def _admin_row(row: dict[str, object]) -> dict[str, object]:
    payload = dict(row.get("payload") or {}) if isinstance(row.get("payload"), dict) else {}
    profile = row.get("business_profile") if isinstance(row.get("business_profile"), dict) else _business_profile(payload)
    selected = row.get("selected_providers") if isinstance(row.get("selected_providers"), list) else list(_selected_providers(payload))
    return {
...
def _business_profile(payload: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "profile_complete": bool(name and industry),
    }
