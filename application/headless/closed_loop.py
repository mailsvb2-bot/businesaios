...
class HeadlessClosedLoopService:
    ...
    def _build_action_payload(*, request: Any, executable_action: ExecutableAction, autonomy_decision: Any, run_id: str, step_index: int) -> dict[str, Any]:
        ...
        return {**payload, "action_id": executable_action.action_id, "intent_id": str(getattr(executable_action, "intent_id", "") or ""), "action_type": executable_action.action_type, "channel": executable_action.channel, "decision_id": executable_action.decision_id, "correlation_id": executable_action.correlation_id, "objective_name": executable_action.objective_name, "tenant_id": getattr(request, "tenant_id", ""), "business_id": getattr(request, "business_id", ""), "goal_id": getattr(request, "goal_id", None), "run_id": run_id, "step_index": int(step_index), "action_category": category, "external_confirmation_mode": external_confirmation_mode}
