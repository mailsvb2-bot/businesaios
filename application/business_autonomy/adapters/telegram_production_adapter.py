...
class TelegramProductionAdapter(BaseStaticChannelAdapter):
    ...
    Live Bot API execution is intentionally not fabricated. Until a credential-
    bound transport is injected, the inherited base supports honest simulation
    and returns a fail-closed verdict for live writes.
    """
    ...
    def discover_capabilities(self, *, identity: ChannelIdentity) -> Sequence[ChannelCapabilityDescriptor]:
        return tuple(super().discover_capabilities(identity=identity))
    ...
    async def execute(self, *, envelope: ChannelExecutionEnvelope, request: BusinessExecutionRequest) -> BusinessExecutionResult:
        ...
        return BusinessExecutionResult(
            ...
            metrics={**dict(result.metrics or {}), 'transport': 'telegram_bot_api', 'transport_configured': False},
