...
def _read_conflict_rows(limit: int) -> tuple[dict[str, Any], ...]:
    ...
    rows.sort(
        key=lambda row: (
            str(row.get("resolved_at_utc") or row.get("acknowledged_at_utc") or ""),
...
def _channel_kind_value(value: object) -> str:
    if isinstance(value, ChannelKind):
        return value.value
    return str(value or "").strip()


def provider_catalog_business_class_rows(*, limit: int = 100) -> tuple[dict[str, Any], ...]:
    """Project available business classes from the sole provider catalog owner."""

    rows: list[dict[str, Any]] = []
    seen: set[str] = set()
    for provider in tuple(provider_map().values()):
        channel_kind = _channel_kind_value(getattr(provider, "channel_kind", ""))
        ...
        rows.append(
            {
                ...
                "persistent_surfaces": list(
                    tuple(getattr(provider, "persistent_surfaces", ()) or ())
                ),
...
class ProviderCatalogFleetReadModel:
    def fleet_metrics(self) -> Mapping[str, Any]:
        ...
        open_conflicts = sum(
            1 for row in conflicts if str(row.get("status") or "open") != "resolved"
        )
        return {
            "businesses_total": 0,
            "healthy_capabilities": len(
                {
                    _channel_kind_value(getattr(item, "channel_kind", ""))
                    for item in providers
                    if _channel_kind_value(getattr(item, "channel_kind", ""))
                }
            ),
            "pending_approvals": 0,
    ...
    def business_class_view(self, *, limit: int = 100) -> Sequence[Mapping[str, Any]]:
        return provider_catalog_business_class_rows(limit=limit)
    ...
    def delayed_outcome_health(self) -> Mapping[str, Any]:
        ...
        return {
            "active_total": len(bridge.list_active()),
            "quarantined_total": int(summary.get("quarantined_total", 0)),
        ...
        return tuple(
            {
                ...
                "quarantined_at_utc": item.quarantined_at_utc,
            }
            for item in bridge.list_quarantined()[: max(0, int(limit))]
        )
        ...
        return tuple(
            {
                ...
                "status": item.status,
                "linked_outcome_ids": list(item.linked_outcome_ids),
                "completed_at_utc": item.completed_at_utc,
            }
            for item in bridge.list_sweep_runs(limit=limit)
        )
    ...
    def delayed_outcome_action_rows(self, *, limit: int = 100) -> Sequence[Mapping[str, Any]]:
        ...
        return tuple(
            ...
            for item in bridge.list_action_ledger(limit=limit)
        )
...
__all__ = ["ProviderCatalogFleetReadModel", "provider_catalog_business_class_rows"]
