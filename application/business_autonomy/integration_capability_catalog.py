...
CAPABILITIES: tuple[IntegrationCapability, ...] = (
    ...
    IntegrationCapability(
        ...
        risk_level='medium',
        owner_text='Канонический SMTP outbound готов как approval-gated provider write; автоматическая реактивация требует consent/unsubscribe/bounce policy.',
        next_required_step='Закрыть consent, unsubscribe, bounce и final-delivery evidence перед автоматической реактивацией.',
    ...
    IntegrationCapability(
        ...
        requires_consent=True,
        risk_level='high',
        owner_text='WhatsApp Cloud inbound использует Meta App Secret HMAC, отдельный verify token и canonical multichannel decoder; outbound ещё не сертифицирован.',
        next_required_step='До write-ready доказать template policy, consent, 24h policy, delivery receipts и approval/evidence guards.',
        evidence=(
            _e('provider_catalog.whatsapp_cloud', 'WhatsApp Cloud credentials include app_secret and verify_token'),
            _e('provider_webhook_runtime.whatsapp_cloud', 'raw-body X-Hub-Signature-256 HMAC and verify-token challenge are enforced'),
            _e('messaging_ingress.whatsapp', 'accepted inbound messages use canonical multichannel decoding'),
        ),
    ),
    *_bridge_messaging_interaction_capabilities(),
    IntegrationCapability(
        ...
        requires_consent=True,
        risk_level='medium',
        owner_text='Канонический SMTP outbound использует SecretVault, approval, durable provider queue и ambiguous-send fail-closed semantics.',
        next_required_step='Добавить inbox/reply ingestion, bounce/unsubscribe и final-delivery receipts без второго email runtime.',
        evidence=(_e('provider_catalog.email_connector', 'email provider exists with send_message operation'),),
    ),
    IntegrationCapability(
