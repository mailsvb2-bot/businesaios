...
def _scenario_proof(step_status: str, exact_sha: str | None) -> dict:
    ...
    if step_status == "NOT_PROVEN":
        return proof
    path = reports_dir() / USER_SCENARIO_EVIDENCE_NAME
    if not path.exists():
    ...
    proof.update(
        status="FAIL" if "FAIL" in {step_status, evidence_status} else (
            "PASS" if step_status == evidence_status == "PASS" else "NOT_PROVEN"),
        evidence_status=evidence_status,
        rust_matrix=evidence.get("rust_matrix"),
...
def release_verdict(report: ExecutionReport) -> dict:
    ...
    scenario_step_status = next((item["status"] for item in steps if item["id"] == _step_ids.user_scenario_gate()), "NOT_PROVEN")
    scenario_proof = _scenario_proof(scenario_step_status, exact_sha)
    ...
    status = "FAIL" if "FAIL" in {gate_status, scenario_proof["status"]} else (
        "PASS" if release_complete and exact_sha and scenario_proof["status"] == "PASS" else "NOT_PROVEN")
    return {
        ...
        "canonical_user_scenarios": scenario_proof,
        "steps": steps,
    }
