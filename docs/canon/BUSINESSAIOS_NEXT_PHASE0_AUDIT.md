...
- **Phase 1 ownership rows proven DONE:** Business (`DistributedBusinessRegistry` lifecycle owner), Organization (`OrganizationRegistry` + EventStore chronology), Person (`PersonRegistry` + EventStore chronology, PII-minimal), Lead (`LeadRegistry` + EventStore chronology, PII-minimal), Deal (`DealRegistry` + EventStore chronology, PII-free commercial lifecycle), Employee (`EmployeeRegistry` scoped Person↔Organization relation), Partner (`PartnerRegistry` scoped Person/Organization party relation), Product (`ProductContract` + manifest/YAML catalog), Offer (`ProductOffer` semantic owner + canonical live catalog mutation owner), Service (`BusinessServiceRegistry` business-offering lifecycle), Hypothesis (`GrowthHypothesisV1` + growth EventStore backlog), Task (`DurableTaskRegistry` + EventStore state machine), Artifact (`ArtifactRegistry` immutable metadata lifecycle), Document (`DocumentRegistry` stable identity over Artifact revisions), Customer (`CustomerRegistry` + EventStore chronology), Decision (sovereign issuer + `DecisionArchive` runtime write boundary), Provider (`provider_catalog` definitions + `FileProviderActivationStore` lifecycle), and Evidence (`storage.evidence_store`). Offer now has one canonical durable catalog-mutation owner shared by governed pricing and offer-patch effects, but remains PARTIAL until semantic Offer shapes are consolidated. Action now preserves new full ActionIntentV1 bodies in canonical EvidenceStore with a read-only projector, but remains PARTIAL until historical Evidence rows without full intent bodies are backfilled. Outcome likewise has canonical EvidenceStore persistence for new full BusinessOutcomeV1 bodies plus a read-only projector, but remains PARTIAL until historical Evidence rows without full outcome bodies are backfilled.
