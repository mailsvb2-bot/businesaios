...
- **DUPLICATE/ambiguous ownership:** Message, Payment, Revenue, Risk. Evidence/Data Lineage (Phase 3) is DONE on `storage.evidence_store`; legacy evidence surfaces are migration/archive or rebuildable mirrors rather than competing owners.
- **Phase 1 ownership rows proven DONE:** Business (`DistributedBusinessRegistry` lifecycle owner), Organization (`OrganizationRegistry` + EventStore chronology), Person (`PersonRegistry` + EventStore chronology, PII-minimal), Lead (`LeadRegistry` + EventStore chronology, PII-minimal), Deal (`DealRegistry` + EventStore chronology, PII-free commercial lifecycle), Order (`contracts.order` + single `OrderStore` namespace with `ClientOutcomeOrder` compatibility specialization and fail-closed legacy migration), Employee (`EmployeeRegistry` scoped Person↔Organization relation), Partner (`PartnerRegistry` scoped Person/Organization party relation), Channel (`ChannelIdentity` persisted in `DistributedBusinessRegistry`), Product (`ProductContract` + manifest/YAML catalog), Offer (`ProductOffer` semantic owner + canonical live catalog mutation owner), Service (`BusinessServiceRegistry` business-offering lifecycle), Campaign (`CampaignRegistry` + EventStore lifecycle), Opportunity (`application.opportunity` + EventStore lifecycle), Asset (`AssetRegistry` + EventStore lifecycle, PII-free and schema-versioned), Resource (`BusinessResourceRegistry` + EventStore lifecycle, PII-free with optional canonical Asset relation), Refund (`billing.recovery_contracts.RefundResult` + `RefundOrchestrator` + schema-versioned platform SQLite recovery store), Goal (`BusinessGoalRegistry` + EventStore lifecycle with immutable hierarchy relation), Constraint (`BusinessConstraintRegistry` + EventStore lifecycle with canonical severity and immutable subject relation), Hypothesis (`GrowthHypothesisV1` + growth EventStore backlog), Task (`DurableTaskRegistry` + EventStore state machine), Artifact (`ArtifactRegistry` immutable metadata lifecycle), Document (`DocumentRegistry` stable identity over Artifact revisions), Customer (`CustomerRegistry` + EventStore chronology), Decision (sovereign issuer + `DecisionArchive` runtime write boundary), Provider (`provider_catalog` definitions + `FileProviderActivationStore` lifecycle), and Evidence (`storage.evidence_store`). Capability is now PARTIAL rather than DUPLICATE: the dead application-level DecisionCore vocabulary/invariant copies are removed, while business-integration, advisory, and execution-route capability semantics remain explicitly scoped until a universal lifecycle owner exists. Action now preserves new full ActionIntentV1 bodies in canonical EvidenceStore with a read-only projector, but remains PARTIAL until historical Evidence rows without full intent bodies are backfilled. Outcome likewise has canonical EvidenceStore persistence for new full BusinessOutcomeV1 bodies plus a read-only projector, but remains PARTIAL until historical Evidence rows without full outcome bodies are backfilled.
