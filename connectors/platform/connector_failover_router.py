...
class ConnectorFailoverRouter:
    ...
    def _run(self, *, tenant_id: str, connector_id: str, operation: str, requested_version: str | None, preferred_provider: str | None, require_write: bool, require_verify: bool, dry_run: bool, timeout_seconds: float | None, trace_id: str | None, retry_context: RetryContext, call: Callable[[ConnectorRegistryEntry], ConnectorResult], phase: str, failover_on_result_codes: tuple[str, ...]) -> ConnectorFailoverResult:
        ...
        for route_index, entry in enumerate(candidates):
            if not self._route_safe_for_request(entry=entry, route_index=route_index, requested_version=requested_version, retry_context=retry_context):
                ...
                if retry_context.write and not retry_context.dry_run and not retry_context.idempotency_key_present:
                    break
                continue
            ...
            if self._circuit_breaker is not None:
                ...
                if not permit.allowed:
                    ...
                    self._record_failure(entry=entry, operation=operation, reason='blocked', metadata={'blocked_until': permit.blocked_until})
                    continue
    ...
    def _route_safe_for_request(self, *, entry: ConnectorRegistryEntry, route_index: int, requested_version: str | None, retry_context: RetryContext) -> bool:
        if route_index == 0 or not retry_context.write or retry_context.dry_run or retry_context.idempotency_key_present:
            return True
        return requested_version is None or str(entry.version) == str(requested_version).strip()
    ...
    def _record_observability(self, *, tenant_id: str, entry: ConnectorRegistryEntry, operation: str, phase: str, status: str, trace_id: str | None, duration_ms: float, fallback_depth: int, payload: Mapping[str, object]) -> None:
        if self._observability is None:
            return
        self._observability.record(ConnectorExecutionEvent(tenant_id=tenant_id, connector_id=entry.connector_id, provider=entry.provider, version=entry.version, operation=f'{phase}:{operation}', status=status, trace_id=trace_id, duration_ms=duration_ms, fallback_depth=fallback_depth, route_index=int(payload.get('route_index', fallback_depth) if isinstance(payload, Mapping) else fallback_depth), attempt=int(payload.get('attempt', 0) if isinstance(payload, Mapping) else 0), breaker_state=None if not isinstance(payload, Mapping) else payload.get('breaker_state'), payload=dict(payload)))
    ...
    def _should_failover_on_result(self, *, result: ConnectorResult, failover_on_result_codes: tuple[str, ...], retry_context: RetryContext) -> bool:
        ...
        if code not in {str(x).strip().lower() for x in failover_on_result_codes}:
            return False
        if not retry_context.write or retry_context.dry_run or retry_context.idempotency_key_present:
            return True
        return False
