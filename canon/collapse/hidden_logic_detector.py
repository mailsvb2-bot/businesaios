        ...
        for node in ast.walk(ast.parse(path.read_text(encoding="utf-8"), filename=str(path))):
            if isinstance(node, ast.ClassDef):
                ...
                if node.name in config.forbidden_decision_method_names:
                    findings.append(HiddenLogicFinding(relpath, node.lineno, node.name, FindingSeverity.CRITICAL, "forbidden decision-emission method exists outside canonical path")); continue
                if any(lowered.startswith(prefix) for prefix in config.hidden_logic_return_hints):
                    for stmt in ast.walk(node):
                        if isinstance(stmt, ast.Return) and isinstance(stmt.value, ast.Dict) and _dict_contains_action_keys(stmt.value, config):
                            findings.append(HiddenLogicFinding(relpath, node.lineno, node.name, FindingSeverity.CRITICAL, "function appears to construct/return final action payload outside canonical decision surface")); break
