...
def _classify_severity(config:
    LegacyCanonConfig, fragments: list[DuplicateFragment]) -> tuple[FindingSeverity, str]:
...
def scan_duplicate_logic(config:
    LegacyCanonConfig) -> tuple[DuplicateCluster, ...]:
    ...
    for path in _iter_python_files(config):
        ...
        for node in ast.walk(ast.parse(source, filename=str(path))):
            if isinstance(node, ast.FunctionDef | ast.AsyncFunctionDef) and _name_is_interesting(config, node.name) and _statement_weight(node.body) >= 3:
                ...
                semantic_hash = _semantic_hash(node)
                buckets.setdefault(("class", semantic_hash), []).append(DuplicateFragment(relpath, node.lineno, "class", node.name, semantic_hash))
