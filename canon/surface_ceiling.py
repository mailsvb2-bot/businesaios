...
def transition_path_marked_python_files(root: Path) -> tuple[str, ...]:
    return tuple(sorted(path.relative_to(root).as_posix() for path in iter_canonical_python_files(root) if any(token in path.relative_to(root).as_posix() for token in _TRANSITION_PATH_TOKENS)))
...
__all__ = ["SURFACE_CEILING", "SurfaceCeiling", "count_path_marked_transition_files", "count_python_files", "is_canonical_source_path", "iter_canonical_python_files", "surface_ceiling_snapshot", "transition_path_marked_python_files"]
