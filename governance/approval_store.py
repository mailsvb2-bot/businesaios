...
def _require_current(current: ApprovalRecord | None, expected: ApprovalRecord | None) -> None:
...
class InMemoryApprovalStore(ApprovalStoreContract):
    ...
    def save(self, record: ApprovalRecord, *, expected: ApprovalRecord | None = None) -> ApprovalRecord:
        ...
        with self._lock:
            ...
            _require_current(self._items.get(approval_id), expected)
            self._items[approval_id] = replace(record)
            return record
    ...
    def list_for_tenant(self, *, tenant_id: str, include_terminal: bool = True) -> tuple[ApprovalRecord, ...]:
        ...
        with self._lock:
            ...
            items = [record for record in self._items.values()
...
class PersistentApprovalStore(ApprovalStoreContract):
    """Durable file-backed approval state with cross-owner atomic updates."""
    ...
    def _flush(self) -> None:
        atomic_write_json(self._path, {"records": [self._record_to_payload(item) for item in self._items.values()]})
    ...
    def _record_from_payload(payload: dict[str, object]) -> ApprovalRecord:
        ...
        decisions = tuple(from_dataclass(ApprovalDecision, item) for item in payload.get("decisions", []))
