...
from adapters.api.fastapi.dependencies import FastAPIDependencyContainer, _resolve_runtime_infra
...
def _project_runtime_registry_to_readiness_orchestrator(registry: object) -> object | None:
    ...
    for required_service in ('event_bus', 'metrics', 'tracer'):
        if required_service not in services:
            services.register(required_service, _ReadinessComponent(required_service, backing=observability))
    for required_component in ('decision_audit_log', 'action_audit_log'):
        if required_component not in components:
            components.register(required_component, _ReadinessComponent(required_component, backing=observability))
...
def _resolve_runtime_orchestrator(dependency_container: FastAPIDependencyContainer | None) -> object | None:
    ...
    services = _first_present_attr(runtime, 'services', 'service_registry') or _first_present_attr(
        runtime_infra, 'services', 'service_registry'
    )
    components = _first_present_attr(runtime, 'components', 'component_registry') or _first_present_attr(
        runtime_infra, 'components', 'component_registry'
    )
...
def create_api_router(*, application_service: object, dependency_container: FastAPIDependencyContainer | None = None) -> APIRouter:
    ...
    tenant_guard = TenantRouteGuard(
        tenant_registry=tenant_registry,
        require_active_tenant=tenant_registry_has_records(tenant_registry),
    ...
    webhook_handlers = WebhookRouteHandlers(
        ...
        audit_log=shared_action_audit_log,
        security_guard=security_bundle.webhook_surface_guard,
    )
    ...
    analytics_ops_handlers = AnalyticsOpsRouteHandlers(
        event_store=telemetry_event_store,
        snapshot_db_path=analytics_snapshot_db_path,
        export_root=analytics_export_root,
        queue_bridge=queue_bridge,
    ) if telemetry_event_store is not None else None
    analytics_signed_export_handlers = AnalyticsSignedExportRouteHandlers(
        event_store=telemetry_event_store,
        manifest_chain_db_path=analytics_manifest_chain_db_path,
        export_root=analytics_export_root,
    ) if telemetry_event_store is not None else None
    ...
    register_control_plane_routes(
        router=router,
        auth_bundle=auth_bundle,
        authz_bundle=authz_bundle,
        tenant_guard=tenant_guard,
        rate_limit_bundle=rate_limit_bundle,
        audit_handlers=audit_handlers,
        approval_handlers=approval_handlers,
        admin_handlers=admin_handlers,
        connector_admin_handlers=connector_admin_handlers,
        provider_admin_handlers=provider_admin_handlers,
        metrics_handlers=metrics_handlers,
        webhook_handlers=webhook_handlers,
        queue_ops_handlers=queue_ops_handlers,
        security_guard=security_bundle.control_plane_guard,
        analytics_ops_handlers=analytics_ops_handlers,
        analytics_signed_export_handlers=analytics_signed_export_handlers,
    )
