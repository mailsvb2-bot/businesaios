...
def _product_fields(value) -> dict:
    return {'business_profile': dict(getattr(value, 'business_profile', None) or {}), 'selected_providers': list(getattr(value, 'selected_providers', ()) or ()), 'integration_plan': list(getattr(value, 'integration_plan', ()) or ()), 'autonomy_mode': str(getattr(value, 'autonomy_mode', 'advisor') or 'advisor'), 'first_value_preview': dict(getattr(value, 'first_value_preview', None) or {}), 'onboarding_progress': dict(getattr(value, 'onboarding_progress', None) or {})}


def _base_response(value) -> dict:
    return {'ok': True, 'intake_id': value.intake_id, 'created_at': value.created_at, 'tenant_id': value.tenant_id, 'business_id': value.business_id, 'user_id': value.user_id, 'onboarding_status': value.onboarding_status, 'next_actions': list(value.next_actions), 'user_functionality': dict(value.user_functionality or {}), 'admin_visibility': dict(value.admin_visibility or {}), 'measurable_outcome': value.outcome, 'write_actions_enabled': False, 'approval_required_before_execution': True, **_product_fields(value)}
...
def _owner_session_payload(record, raw_key: str, *, tenant_id: str, business_id: str) -> dict[str, object]:
    return {'api_key': raw_key, 'tenant_id': tenant_id, 'business_id': business_id, 'expires_at': None if record.expires_at is None else record.expires_at.isoformat(), 'storage': 'session_only'}
...
def _set_owner_resume_cookie(*, response: Response, request: Request, raw_key: str) -> None:
    response.set_cookie(key=_OWNER_RESUME_COOKIE, value=raw_key, max_age=_OWNER_RESUME_TTL_SECONDS, httponly=True, secure=str(request.url.scheme).lower() == 'https', samesite='strict', path='/')
...
def register_public_site_routes(*, router, enforce_public_security, auth_bundle=None, tenant_registry=None) -> None:
    ...
    async def public_site_cta_start(http_request: Request, response: Response) -> dict:
        ...
        result, owner_session, policy = service.submit(payload=payload), None, api_key_policy()
        if tenant_registry is not None and policy is not None:
            ...
            record, raw_key = policy.issue_owner_session(tenant_id=result.tenant_id, business_id=result.business_id, subject=result.user_id, display_name=display_name)
            _, raw_resume_key = policy.issue_owner_resume_session(tenant_id=result.tenant_id, business_id=result.business_id, intake_id=result.intake_id, subject=result.user_id, display_name=display_name, ttl_seconds=_OWNER_RESUME_TTL_SECONDS)
    ...
    async def public_site_cta_status(http_request: Request, response: Response, intake_id: str) -> dict:
        ...
        status_payload, owner_session, policy = service.get_status(intake_id=intake_id), None, api_key_policy()
        ...
        if status_payload.found and policy is not None and resume_key:
            resumed = policy.resume_owner_session(resume_key=resume_key, intake_id=status_payload.intake_id, tenant_id=status_payload.tenant_id, business_id=status_payload.business_id)
