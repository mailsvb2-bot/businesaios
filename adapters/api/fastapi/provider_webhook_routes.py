...
def register_provider_webhook_routes(*, router, provider_admin_handlers) -> None:
    ...
    async def public_provider_webhook_ingest(tenant_id: str, business_id: str, provider_key: str, request: Request) -> dict | Response:
        ...
        result = provider_admin_handlers.ingest_provider_webhook(payload={'tenant_id': tenant_id, 'business_id': business_id, 'provider_key': provider_key, 'headers': headers, 'body': raw_body, 'event_key': event_key, 'topic': topic, 'owner_id': 'public_provider_webhook'})
