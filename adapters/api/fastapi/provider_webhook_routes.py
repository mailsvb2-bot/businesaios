...
def register_provider_webhook_routes(*, router, provider_admin_handlers) -> None:
    ...
    async def public_provider_webhook_ingest(tenant_id: str, business_id: str, provider_key: str, request: Request) -> dict | Response:
        ...
        parsed = ProviderPayloadNormalizers.parse_webhook_json(raw_body)
        nested_event = parsed.get('event') if isinstance(parsed.get('event'), Mapping) else {}
        event_key = (
            str(headers.get('X-Event-Id') or headers.get('X-Shopify-Webhook-Id') or headers.get('X-Request-Id') or '').strip()
            or str(parsed.get('event_id') or parsed.get('id') or nested_event.get('client_msg_id') or nested_event.get('event_ts') or '').strip()
            or request.headers.get('x-amz-request-id', '')
            or request.headers.get('cf-ray', '')
            or 'payload-digest-fallback'
        )
        topic = (
            str(headers.get('X-Topic') or headers.get('X-Shopify-Topic') or headers.get('X-Webhook-Topic') or '').strip()
            or str(nested_event.get('type') or parsed.get('type') or '').strip()
        )
        ...
        if provider_key in {'vk_messaging', 'max_messaging', 'slack_messaging'} and result.get('status') == 'invalid_signature':
