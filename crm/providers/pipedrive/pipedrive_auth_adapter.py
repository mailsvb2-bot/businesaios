...
class PipedriveAuthAdapter:
    def __init__(self, *, token_store: CrmOAuthTokenStore, client_id: str, client_secret: str, api_config: PipedriveApiConfig | None = None) -> None:
        ...
        self._provider_config = CrmOAuthProviderConfig(
            ...
            token_url=f'{self._api_config.oauth_base_url}/oauth/token',
            client_id=client_id,
    ...
    def authorized_client(self, *, secret_ref: str, company_domain: str) -> CrmHttpClient:
        token = self._oauth_service.ensure_active_token(config=self._provider_config, secret_ref=secret_ref)
        return CrmHttpClient(base_url=self._api_config.api_base_template.format(company_domain=company_domain), default_headers={'Authorization': f'Bearer {token.access_token}'})
