...
from crm.providers.pipedrive.pipedrive_capability_descriptor import build_pipedrive_capability_descriptor
from crm.providers.pipedrive.pipedrive_connection_adapter import ProviderConnectionAdapter
from crm.providers.pipedrive.pipedrive_contact_adapter import PipedriveContactAdapter
from crm.providers.pipedrive.pipedrive_deal_adapter import PipedriveDealAdapter
from crm.providers.pipedrive.pipedrive_note_adapter import PipedriveNoteAdapter
from crm.providers.pipedrive.pipedrive_pipeline_adapter import DEFAULT_PIPELINE, PipedrivePipelineAdapter
from crm.providers.pipedrive.pipedrive_verification_adapter import PipedriveVerificationAdapter


class PipedriveConnector(CrmConnector):
    def __init__(self, *, token_store: CrmOAuthTokenStore | None = None, client_id: str | None = None, client_secret: str | None = None, api_config: PipedriveApiConfig | None = None) -> None:
        self.provider = CrmProvider(provider_key='pipedrive', display_name='Pipedrive', capability_descriptor=build_pipedrive_capability_descriptor())
        self._store = CrmProviderStore('pipedrive', default_pipelines=(DEFAULT_PIPELINE,))
        self._api_config = api_config or PipedriveApiConfig()
        self._auth_adapter = None
        if client_id and client_secret:
            self._auth_adapter = PipedriveAuthAdapter(token_store=token_store or InMemoryCrmOAuthTokenStore(), client_id=client_id, client_secret=client_secret, api_config=self._api_config)
        ...
        self._pipeline_adapter = PipedrivePipelineAdapter(self._store, auth_adapter=self._auth_adapter, api_config=self._api_config)
        self._contact_adapter = PipedriveContactAdapter(self._store, auth_adapter=self._auth_adapter, api_config=self._api_config)
        self._deal_adapter = PipedriveDealAdapter(self._store, auth_adapter=self._auth_adapter, api_config=self._api_config)
        self._note_adapter = PipedriveNoteAdapter(self._store, auth_adapter=self._auth_adapter, api_config=self._api_config)
        self._verification_adapter = PipedriveVerificationAdapter(self._store, auth_adapter=self._auth_adapter, api_config=self._api_config)
    ...
    def _company_domain(self, connection: CrmConnectionRef) -> str:
        return str(connection.metadata.get('company_domain') or '').strip()

    def _live(self, connection: CrmConnectionRef) -> bool:
        return bool(self._auth_adapter is not None and connection.secret_ref and self._company_domain(connection) and bool(connection.metadata.get('live_api')))
    ...
    def list_pipelines(self, connection: CrmConnectionRef):
        if self._live(connection):
            return self._pipeline_adapter.list_pipelines(secret_ref=connection.secret_ref, company_domain=self._company_domain(connection))
    ...
    def upsert_pipeline(self, connection: CrmConnectionRef, pipeline: CrmPipeline, *, idempotency_key: str) -> dict[str, object]:
        if self._live(connection):
            return self._pipeline_adapter.upsert(connection, pipeline, idempotency_key=idempotency_key, company_domain=self._company_domain(connection))
    ...
    def upsert_contact(self, connection: CrmConnectionRef, contact: CrmContact, *, idempotency_key: str) -> dict[str, object]:
        if self._live(connection):
            return self._contact_adapter.upsert(contact=contact, secret_ref=connection.secret_ref, company_domain=self._company_domain(connection), idempotency_key=idempotency_key)
    ...
    def upsert_deal(self, connection: CrmConnectionRef, deal: CrmDeal, *, idempotency_key: str) -> dict[str, object]:
        if self._live(connection):
            return self._deal_adapter.upsert(deal=deal, secret_ref=connection.secret_ref, company_domain=self._company_domain(connection), idempotency_key=idempotency_key)
    ...
    def append_note(self, connection: CrmConnectionRef, note: CrmNote, *, idempotency_key: str) -> dict[str, object]:
        if self._live(connection):
            return self._note_adapter.append(note=note, secret_ref=connection.secret_ref, company_domain=self._company_domain(connection), idempotency_key=idempotency_key)
    ...
    def verify_write(self, connection: CrmConnectionRef, request: CrmVerificationRequest) -> CrmVerificationResult:
        if self._live(connection):
            return self._verification_adapter.verify(request=request, secret_ref=connection.secret_ref, company_domain=self._company_domain(connection))
