...
def _sync_request(*args: Any, **kwargs: Any) -> Any:
    sync_request = import_internal_attr(
    ...
    def _send_once(
        ...
        *,
        ...
        if result.error_kind == "transport_error":
            ...
            raise CrmTransportError(
                f"CRM transport error: {detail}",
                context=context,
            )
        if result.error_kind == "http_error":
            ...
            if status_code in {401, 403}:
                raise CrmAuthenticationError(
                    f"CRM authentication failed: {status_code}",
                    context=context,
                )
            if status_code == 429:
                raise CrmRateLimitError(
                    "CRM rate limit reached",
                    context=context,
                )
            raise CrmResponseError(
                f"CRM response error: {status_code}",
                context=context,
            )
        ...
        return CrmHttpResponse(
            ...
            json_body=(
                ...
                else self._try_json(result.text)
            ),
            text=result.text,
        )
    ...
    def _build_body(
        ...
        populated = sum(
            ...
            for value in (
                ...
                request_spec.form_body,
                request_spec.raw_body,
            )
        )
        if populated > 1:
            raise ValueError(
                "Only one of json_body, form_body, or raw_body may be supplied"
            )
