...
def _mapping(value: object, code: str) -> Mapping[str, Any]:
    return value if isinstance(value, Mapping) else _fail(code)
...
def _minor(value: object, offer_id: str) -> int:
    ...
    return int(minor)
def _external(raw: Mapping[str, Any], root: Path, ref: str) -> OfferCatalog:
    ...
    catalog_path = (catalog_root / f"{domain}.yaml").resolve()
    if descriptor_path.parent != root or catalog_path.parent != catalog_root:
    ...
    for raw_offer in items:
        ...
        title = next((str(v.get("title") or "").strip() for v in ([variants.get("a")] + list(variants.values())) if isinstance(v, Mapping) and str(v.get("title") or "").strip()), "")
        ...
        offers.append(Offer(offer_id=offer_id, title=str(item.get("title") or "").strip() or title or offer_id, price_minor=_minor(item["base_price_rub"], offer_id), currency="RUB", period_days=period_days, metadata={"offer_catalog_ref": ref, **{k: dict(v) for k, v in structured.items()}}))
...
def _legacy(raw: Mapping[str, Any]) -> OfferCatalog:
    ...
    for item in oc.get("offers") if isinstance(oc.get("offers"), list) else []:
        if not isinstance(item, dict) or not str(item.get("offer_id") or "").strip():
            continue
        try: price, period = int(item.get("price_minor") or 0), None if item.get("period_days") is None else int(item["period_days"])
        except (TypeError, ValueError): price, period = 0, None
        tags = item.get("tags"); offers.append(Offer(offer_id=str(item["offer_id"]).strip(), title=str(item.get("title") or item["offer_id"]).strip(), price_minor=price, currency=str(item.get("currency") or "RUB").strip() or "RUB", period_days=period, tags=tuple(str(x) for x in tags if str(x)) if isinstance(tags, (list, tuple)) else (), metadata=item.get("metadata") if isinstance(item.get("metadata"), dict) else {}))
