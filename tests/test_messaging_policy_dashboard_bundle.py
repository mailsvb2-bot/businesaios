...
def test_dashboard_bundle_returns_json():
    ...
    event_store.append(build_event(tenant_id='t1', user_id='u1', decision_id='d1', correlation_id='c1', event_type='messaging_message_delivered', payload={'channel': 'sms'}, created_at='2026-03-01T10:01:00+00:00'))
    event_store.append(build_event(tenant_id='t1', user_id='u1', decision_id='d1', correlation_id='c1', event_type='messaging_policy_execution_finished', payload={'selected_channel': 'sms', 'terminal_reason': '', 'attempts_count': 1}, created_at='2026-03-01T10:02:00+00:00'))
    trace_bundle = build_messaging_policy_trace_search_bundle(event_store=event_store)
    bundle = build_messaging_policy_dashboard_bundle(trace_search_service=trace_bundle.search_service)
    out = bundle.json(tenant_id='t1', user_id='', date_from='', date_to='', limit=500)
    assert out.status_code == 200
    assert out.body['dashboard']['traces_total'] == 1
    assert out.body['dashboard']['selected_channel_counts'][0]['channel'] == 'sms'
