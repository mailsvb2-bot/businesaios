...
class _Gateway:
    ...
    def propose(self, **kwargs):
        ...
        return "p-" + str(len(self.calls))


def test_autopilot_engine_queues_guarded_apply_via_gateway():
    sink = _Sink()
    gw = _Gateway()
    ...
    out = asyncio.run(engine.run(tenant_id="t1", platform="tg", account_id="a1", decision_id="d1", correlation_id="c1"))
    assert out.ok is True
    assert out.queued == 2
    assert out.blocked == 0
    assert len(gw.calls) == 2
    assert any(e.get("event_type") == "ads_autopilot_apply_queued" for e in sink.events)
