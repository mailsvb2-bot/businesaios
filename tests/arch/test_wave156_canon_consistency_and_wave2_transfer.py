...
def test_api_wave2_moves_router_and_health_owners_out_of_interfaces() -> None:
    ...
    shim_health_path = Path("interfaces/api/health_handler.py")
    assert "CANON_API_FASTAPI_ROUTER_FINAL_OWNER = True" in owner_router
    assert "build_runtime_api_bundle(" in owner_router
    assert 'single owner implementation in adapters.api.fastapi.router_adapter' in shim_router
    assert 'from adapters.api.fastapi.router_adapter import *' in shim_router
    assert not shim_health_path.exists()
