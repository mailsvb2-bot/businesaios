...
FACTS = Path("application/person/facts.py")
...
def test_business_autonomy_bootstrap_wires_person_to_existing_event_store() -> None:
    ...
    assert "PersonRegistry(event_store=customer_event_store, idempotency_store=distributed['idempotency'])" in bootstrap
    assert "service._person_registry = person_registry" in bootstrap
