...
class _Log:
    def info(self, *_args: object, **_kwargs: object) -> None: return
    def warning(self, *_args: object, **_kwargs: object) -> None: return
...
def _prepare_bootstrap_env(tmp_path: Path) -> tuple[Path, Path]:
    ...
    registry.register(TenantRecord(tenant_id="production-smoke", display_name="Production smoke", status=TenantStatus.ACTIVE))
    env_file = tmp_path / "api.env"
    env_file.write_text("\n".join((
        "APP_ENV=prod", f"DATA_DIR={tmp_path / 'runtime'}",
        "API_CONTROL_PLANE_API_KEY_PEPPER=test-production-pepper",
        "BUSINESAIOS_API_KEY_STORE_BACKEND=file", f"BUSINESAIOS_API_KEY_STORE_PATH={api_store}",
        "BUSINESAIOS_TENANT_REGISTRY_BACKEND=file", f"BUSINESAIOS_TENANT_REGISTRY_PATH={tenant_store}",
        "CONTROL_PLANE_API_KEY=", "SMOKE_TENANT_ID=", "PRICING_VERSION=old-approved-version", "",
    )), encoding="utf-8")
    env_file.chmod(0o640)
    return env_file, api_store


def test_strict_production_never_accepts_pricing_override_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="requires PRICING_VERSION to be set"):
        enforce_pricing_versioning_or_raise(pricing_config={"currency": "RUB", "price_rub": 1000}, production_strict=True, log=_Log())


def test_strict_pricing_fingerprint_defaults_to_canonical_data_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_canonical_bootstrap_atomically_binds_explicit_pricing_version(tmp_path: Path) -> None:
    ...
    result = bootstrap.bootstrap_production_control_plane(tenant_id="production-smoke", env_file=env_file, pricing_version="pricing-2026-08-19-rub")
    _, values = bootstrap.read_environment_file(env_file)
    assert values["PRICING_VERSION"] == result.pricing_version == "pricing-2026-08-19-rub"
...
def test_invalid_pricing_version_fails_before_credential_or_env_mutation(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="PROD_STRICT_PRICING_VERSION_INVALID"):
        bootstrap.bootstrap_production_control_plane(tenant_id="production-smoke", env_file=env_file, pricing_version="v1")
...
def test_host_lifecycle_owns_production_tenant_pricing_and_enabled_connector_readiness() -> None:
    ...
    for token in (
        'APPROVED_PRICING_VERSION="${PRICING_VERSION:-}"', "unset SMOKE_TENANT_ID PRICING_VERSION",
        "build_default_tenant_policy_store", "policy_store.require(smoke_tenant)",
        ...
        'systemctl reset-failed "$TELEGRAM_SERVICE"', '--pricing-version "$APPROVED_PRICING_VERSION"',
    ):
...
def test_production_template_has_single_persistent_pricing_and_policy_surfaces() -> None:
    ...
    for token in (
        "PRODUCTION_STRICT_MODE=1", "PRICING_VERSION=",
