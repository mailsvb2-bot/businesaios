...
def test_production_boot_blocks_when_postgres_live_is_not_ready(monkeypatch) -> None:
    _write_artifact(
        "postgres_contract.json",
        {"artifact": "postgres_contract", "status": "ready", "violations": [], "claims_production_ready": False},
    )
    _write_artifact(
        "postgres_live.json",
        {"artifact": "postgres_live", "status": "advisory_only", "warnings": ["postgres_runtime_not_declared"], "claims_production_ready": False},
    )
    ...
    assert ok is False
    assert "postgres_live_not_ready" in message
    assert payload["status"] == "blocked"
    assert payload["production_profile"] is True
    assert payload["postgres_contract"]["status"] == "ready"
    assert payload["postgres_live"]["status"] == "advisory_only"
    assert "postgres_live_not_ready" in payload["violations"]
    assert payload["claims_production_ready"] is False


def test_production_boot_allows_non_production_with_advisory_postgres_live(monkeypatch) -> None:
    _write_artifact(
        "postgres_contract.json",
        {"artifact": "postgres_contract", "status": "advisory_only", "warnings": ["postgres_runtime_not_declared"], "claims_production_ready": False},
    )
    _write_artifact(
        "postgres_live.json",
        {"artifact": "postgres_live", "status": "advisory_only", "warnings": ["postgres_runtime_not_declared"], "claims_production_ready": False},
