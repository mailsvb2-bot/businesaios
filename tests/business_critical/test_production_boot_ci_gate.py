...
def test_production_boot_gate_is_registered() -> None:
    ...
    assert [step.name for step in plan.steps] == ["assert-project-shape", "doctor-check", "production-boot"]
...
def test_release_plans_include_production_boot_before_release_artifacts() -> None:
    ...
    assert prerelease_steps.index("production-boot") < prerelease_steps.index("verify-release")
