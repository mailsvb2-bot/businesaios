...
def test_evidence_ref_tampering_breaks_signed_decision_payload():
    keyring = Keyring({"k1": {"secret": b"s1", "revoked": False}}, "k1")
    decision = Decision(
        decision_id="d-evidence", issuer_id="businesaios-core", issued_at_ms=100, expires_at_ms=200,
        policy_id="p1", action="send_message@v1",
        payload={"meta": {"world_model_meta": {"evidence_refs": ["evidence-1"]}}},
