...
from tools.migrate_key_provider_storage import apply_migration, build_migration_plan, main


def _inline_key_payload(secret: bytes) -> dict[str, object]:
...
def test_dry_run_is_read_only(monkeypatch, tmp_path, capsys) -> None:
    ...
    payload = {"records": [], "keys": [_inline_key_payload(b"inline-secret-key")]}
    ...
    assert not Path(str(vault_path) + ".inline-keys-v1.bak").exists()


def test_apply_moves_keys_to_aead_provider_and_keeps_backup(monkeypatch, tmp_path) -> None:
    master = bytes(range(32))
    monkeypatch.setenv(
        "BUSINESAIOS_KEY_PROVIDER_MASTER_KEY_B64",
        base64.b64encode(master).decode("ascii"),
    ...
    vault_path = tmp_path / "secret_vault.json"
    key_provider_path = tmp_path / "key_provider.json"
    secret = b"inline-secret-key"
    payload = {
        "records": [{"ref": {"tenant_id": "tenant-a"}}],
        "keys": [_inline_key_payload(secret)],
    }
    vault_path.write_text(json.dumps(payload), encoding="utf-8")
    plan = build_migration_plan(vault_path=vault_path, key_provider_path=key_provider_path)
    ...
    assert provider_payload["records"][0]["key_envelope_version"] == "BAIOS-KE2"
