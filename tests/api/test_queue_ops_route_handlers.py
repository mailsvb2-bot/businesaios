...
def test_queue_ops_route_handlers_build_view_and_audit(tmp_path):
    ...
    view = handlers.get_queue_ops_view(tenant_id='tenant-a', queue_name='ops', actor_id='operator-1', request_id='req-view', now=datetime(2026, 1, 1, 0, 0, 0, tzinfo=timezone.utc))
    ...
    result = handlers.execute_remediation_hook(tenant_id='tenant-a', queue_name='ops', hook_code='open_queue_history', actor_id='operator-1', request_id='req-exec', now=datetime(2026, 1, 1, 0, 1, 0, tzinfo=timezone.utc))
...
def test_queue_ops_route_handlers_timeline_and_filters(tmp_path):
    ...
    handlers.execute_remediation_hook(tenant_id='tenant-a', queue_name='ops', hook_code='open_queue_history', actor_id='operator-1', request_id='req-exec', now=datetime(2026, 1, 1, 0, 1, 0, tzinfo=timezone.utc))
...
def test_queue_ops_route_handlers_builds_evidence_timeline_and_consistency(tmp_path):
    ...
    handlers = QueueOpsRouteHandlers(store=store)
    dispatcher.dispatch(JobDispatchRequest(tenant_id='tenant-a', job_id='job-1', queue_name='ops', job_type='demo', payload={}, dedupe_key='d1'))
    view = handlers.get_queue_ops_view(tenant_id='tenant-a', queue_name='ops', now=datetime(2026, 1, 1, 0, 0, 0, tzinfo=timezone.utc))
...
def test_queue_ops_route_handlers_staleness_consistency_degrades(tmp_path):
    ...
    stale_freshness = handlers._build_data_freshness(
        monitor_report=type('M', (), {'sampled_at': datetime(2026, 1, 1, 0, 0, 0, tzinfo=timezone.utc)})(),
        rollup_summary=None,
        now=now,
    )
...
def test_queue_ops_route_handlers_recent_alerts_are_latest_first(tmp_path):
    ...
    handlers.alert_sink.publish((
        QueueAlert(tenant_id='tenant-a', queue_name='ops', code='older', severity='warning', message='older', created_at=datetime(2026, 1, 1, 0, 0, 0, tzinfo=timezone.utc)),
        QueueAlert(tenant_id='tenant-a', queue_name='ops', code='newer', severity='warning', message='newer', created_at=datetime(2026, 1, 1, 0, 1, 0, tzinfo=timezone.utc)),
    ))
...
def test_queue_ops_route_handlers_route_metadata_datetime_and_set_are_sanitized(tmp_path):
    ...
    handlers._record_route_event(
        ...
        source='control_plane',
        status='ok',
        metadata={'seen_at': datetime(2026, 1, 1, 0, 2, 0, tzinfo=timezone.utc), 'items': {'b', 'a'}},
        now=datetime(2026, 1, 1, 0, 2, 0, tzinfo=timezone.utc),
    )
...
def test_queue_ops_route_handlers_normalize_actor_and_request_fields(tmp_path):
    ...
    handlers._record_route_event(
        ...
        now=datetime(2026, 1, 1, 0, 3, 0, tzinfo=timezone.utc),
    )
    audit = handlers.list_remediation_audit(tenant_id='tenant-a', queue_name='ops', action='custom_event')
