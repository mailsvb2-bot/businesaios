...
from scripts.ci import step_ids, step_registry
...
def test_required_ci_build_release_gates_see_baseline_contract() -> None:
    assert required_gates() == ("fast", "full", "pre-release", "release")
    for gate in required_gates():
        steps = _gate_steps(gate)
        assert "baseline-contract" in steps
        assert steps.index("baseline-contract") < steps.index("import-smoke")
    release_steps = _gate_steps("release")
    assert release_steps.index("baseline-contract") < release_steps.index("verify-release")
    assert release_steps.index("baseline-contract") < release_steps.index("build-artifact")


def test_baseline_contract_step_is_registered_lazily() -> None:
    assert step_ids.baseline_contract() == "baseline-contract"
    assert "baseline-contract" in step_registry.all_step_names()
    assert step_registry.handler_for_step("baseline-contract").__name__.startswith("lazy_step_baseline_contract")


def test_baseline_contract_step_passes_on_current_tree() -> None:
