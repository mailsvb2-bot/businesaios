...
from acquisition import AcquisitionHeadlessEntrypoint
from headless.acquisition_execution import (
...
def test_headless_acquisition_execution_marker_is_enabled() -> None:
    assert CANON_HEADLESS_ACQUISITION_EXECUTION is True


def test_create_headless_acquisition_execution_returns_boundary_object() -> None:
    execution = create_headless_acquisition_execution()
    assert isinstance(execution, HeadlessAcquisitionExecution)
    assert isinstance(execution.entrypoint, AcquisitionHeadlessEntrypoint)


def test_execute_headless_acquisition_returns_view_model() -> None:
    vm = execute_headless_acquisition(_payload())
