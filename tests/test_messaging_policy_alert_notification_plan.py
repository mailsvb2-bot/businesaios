...
def test_notification_planner_builds_items():
    ...
    subscriptions = (AlertSubscriptionRecord(tenant_id="t1", recipient_user_id="ceo-1", channel="email", min_level="warn", business_id="business-a", enabled=True, code_filters=(), user_scope=()),)
    ...
    assert plan.items[0].business_id == "business-a"
