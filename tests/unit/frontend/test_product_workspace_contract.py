...
def test_owner_workspace_resumes_server_session_without_persisting_owner_key_in_web_storage() -> None:
    ...
    assert 'aria-label={`Открыть бизнес ${item.name || "Бизнес"}`}' in app
...
def test_provider_setup_ui_distinguishes_public_config_from_secrets_and_explains_native_messaging_paths() -> None:
    ...
    assert all(token in app for token in (
        "PROVIDER_CONNECTION_GUIDANCE", "vk_messaging", "max_messaging", "slack_messaging", "discord_messaging", "instagram_messaging", "messenger_messaging", "line_messaging", "viber_messaging",
        "credentialInputType", "secret_kind", 'type={credentialInputType(field)}', "Webhook URL", "Application Public Key",
        "Slack Signing Secret", "Bridge Webhook Secret", "Instagram Access Token", "Page Access Token", "LINE Official Account", "Viber Bot", "Что реально доступно", "Gateway-подключение и отправка сообщений",
    ))
