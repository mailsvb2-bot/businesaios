...
def test_platform_control_center_reports_advisory_not_live_ready(tmp_path):
    ...
    widget_runtime = service.build_widget_runtime(
        tenant_id="tenant-demo",
        business_id="biz-demo",
    )
    ...
    assert {row["state"] for row in stop_conditions["rows"]} == {
        "open"
    }


def test_platform_control_center_does_not_emit_fake_remediation_patch(
    tmp_path,
):
    service = PlatformControlCenterService.for_repo(tmp_path)
...
def test_snapshot_and_maturity_surfaces_do_not_claim_unwired_proof(
    tmp_path,
):
    service = PlatformControlCenterService.for_repo(tmp_path)
