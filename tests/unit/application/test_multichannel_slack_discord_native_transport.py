...
def test_slack_discord_bindings_remain_prepared_only_even_with_vault() -> None:
    ...
    assert {'slack_messaging', 'discord_messaging'} <= set(transports)
    for key in ('slack_messaging', 'discord_messaging'):
        result = transports[key].execute(provider=provider_map()[key], tenant_id='t', business_id='b', operation='health_probe', payload={})
        assert result['_prepared_only'] is True
        assert result['transport_binding']['live_ready'] is False
