...
def test_expense_metadata_propagates_and_same_key_replay_rejects_change() -> None:
    registry, events = _registry()
    record_metadata = { "actor_id": "owner-1", "decision_id": "decision-record", "evidence_ids": ("receipt-1",), }
    recorded = registry.record( tenant_id="tenant", business_id="business", expense_id="expense-1", idempotency_key="record-meta", amount="125.50", currency="EUR", category="tools", incurred_at_ms=100, recorded_at_ms=120, event_metadata=record_metadata, )
    assert registry.record( tenant_id="tenant", business_id="business", expense_id="expense-1", idempotency_key="record-meta", amount="125.500", currency="eur", category="tools", incurred_at_ms=100, recorded_at_ms=999, event_metadata=record_metadata, ) == recorded
    rows = list(events.iter_events( tenant_id="tenant", start_ms=0, event_type=BUSINESS_FACT_EVENT_TYPE ))
    assert len(rows) == 1
    ...
    with pytest.raises(ValueError, match="event metadata"):
        registry.record( tenant_id="tenant", business_id="business", expense_id="expense-1", idempotency_key="record-meta", amount="125.50", currency="EUR", category="tools", incurred_at_ms=100, recorded_at_ms=120, event_metadata={**record_metadata, "actor_id": "owner-2"}, )
    void_metadata = {"actor_id": "owner-1", "decision_id": "decision-void"}
    voided = registry.void( tenant_id="tenant", business_id="business", expense_id="expense-1", idempotency_key="void-meta", occurred_at_ms=200, event_metadata=void_metadata, )
    assert registry.void( tenant_id="tenant", business_id="business", expense_id="expense-1", idempotency_key="void-meta", occurred_at_ms=999, event_metadata=void_metadata, ) == voided
    with pytest.raises(ValueError, match="event metadata"):
        registry.void( tenant_id="tenant", business_id="business", expense_id="expense-1", idempotency_key="void-meta", occurred_at_ms=200, event_metadata={**void_metadata, "actor_id": "owner-2"}, )
