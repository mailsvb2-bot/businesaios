...
def test_deal_metadata_propagates_and_replay_rejects_change() -> None:
    registry, events = _registry()
    create_metadata = { "actor_id": "owner-1", "decision_id": "decision-create", "correlation_id": "deal-flow-1", "evidence_ids": ("evidence-create",), }
    created = registry.create( tenant_id="tenant-1", business_id="business-1", deal_id="deal-1", idempotency_key="create-meta", pipeline_key="sales", stage_key="new", amount_minor=12500, currency="RUB", occurred_at_ms=100, event_metadata=create_metadata, )
    assert registry.create( tenant_id="tenant-1", business_id="business-1", deal_id="deal-1", idempotency_key="create-meta", pipeline_key="sales", stage_key="new", amount_minor=12500, currency="RUB", occurred_at_ms=999, event_metadata=create_metadata, ) == created
    assert len(events.events) == 1
    ...
    with pytest.raises(ValueError, match="event metadata"):
        registry.create( tenant_id="tenant-1", business_id="business-1", deal_id="deal-1", idempotency_key="create-meta", pipeline_key="sales", stage_key="new", amount_minor=12500, currency="RUB", occurred_at_ms=100, event_metadata={**create_metadata, "actor_id": "owner-2"}, )
    update_metadata = {"actor_id": "owner-1", "decision_id": "decision-update"}
    updated = registry.update( tenant_id="tenant-1", business_id="business-1", deal_id="deal-1", idempotency_key="update-meta", stage_key="qualified", amount_minor=15000, occurred_at_ms=200, event_metadata=update_metadata, )
    assert registry.update( tenant_id="tenant-1", business_id="business-1", deal_id="deal-1", idempotency_key="update-meta", stage_key="qualified", amount_minor=15000, occurred_at_ms=999, event_metadata=update_metadata, ) == updated
    with pytest.raises(ValueError, match="event metadata"):
        registry.update( tenant_id="tenant-1", business_id="business-1", deal_id="deal-1", idempotency_key="update-meta", stage_key="qualified", amount_minor=15000, occurred_at_ms=200, event_metadata={**update_metadata, "actor_id": "owner-2"}, )
    archive_metadata = {"actor_id": "owner-1", "decision_id": "decision-archive"}
    archived = registry.archive( tenant_id="tenant-1", business_id="business-1", deal_id="deal-1", idempotency_key="archive-meta", occurred_at_ms=300, event_metadata=archive_metadata, )
    assert registry.archive( tenant_id="tenant-1", business_id="business-1", deal_id="deal-1", idempotency_key="archive-meta", occurred_at_ms=999, event_metadata=archive_metadata, ) == archived
    with pytest.raises(ValueError, match="event metadata"):
        registry.archive( tenant_id="tenant-1", business_id="business-1", deal_id="deal-1", idempotency_key="archive-meta", occurred_at_ms=300, event_metadata={**archive_metadata, "actor_id": "owner-2"}, )
