...
def test_deal_metadata_propagates_and_replay_rejects_change() -> None:
    ...
    create_metadata = {
        ...
        "correlation_id": "deal-flow-1",
        "evidence_ids": ("evidence-create",),
    }
    created = registry.create(
        tenant_id="tenant-1", business_id="business-1", deal_id="deal-1",
        idempotency_key="create-meta", pipeline_key="sales", stage_key="new",
        amount_minor=12500, currency="RUB", occurred_at_ms=100,
    ...
    updated = registry.update(
        tenant_id="tenant-1", business_id="business-1", deal_id="deal-1",
        idempotency_key="update-meta", stage_key="qualified", amount_minor=15000,
