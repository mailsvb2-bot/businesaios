...
def test_manifest_chain_links_previous_export(tmp_path) -> None:
    with SqliteAnalyticsManifestChainStore(str(tmp_path / 'manifest_chain.db')) as chain_store:
        ...
        first = service.export_signed_bundle(
            export_dir='chain',
            export_id='exp-1',
            tenant_id='tenant-1',
            bundle={'dashboard': {'overall_state': 'healthy'}},
        )
        second = service.export_signed_bundle(
            export_dir='chain',
            export_id='exp-2',
            tenant_id='tenant-1',
            bundle={'dashboard': {'overall_state': 'warning'}},
        )
