...
def test_invoice_transition_replays_are_durable_metadata_checked_and_request_safe() -> None:
    ...
    issued = registry.issue(
        ...
        business_id="business-a",
        invoice_id="inv-1",
        idempotency_key="issue-meta",
        issued_at=issued_at,
        due_at=issued_at + timedelta(days=14),
        occurred_at_ms=200,
        event_metadata=issue_metadata,
    )
    ...
    partial = registry.record_payment(
        ...
        business_id="business-a",
        invoice_id="inv-1",
        idempotency_key="pay-meta",
        amount_minor=400,
        occurred_at_ms=300,
        event_metadata=payment_metadata,
    )
    before_replay_count = len(events.events)
    assert registry.record_payment(
        tenant_id="tenant-a",
        business_id="business-a",
        invoice_id="inv-1",
        idempotency_key="pay-meta",
    ...
    with pytest.raises(ValueError, match="event metadata"):
        registry.record_payment(
            ...
            amount_minor=400,
            occurred_at_ms=999,
            event_metadata={**payment_metadata, "actor_id": "owner-2"},
        )
    ...
    credited = registry.credit(
        ...
        business_id="business-a",
        invoice_id="inv-1",
        idempotency_key="credit-meta",
        occurred_at_ms=400,
        event_metadata=credited_metadata,
    )
    assert registry.credit(
        tenant_id="tenant-a",
        business_id="business-a",
        invoice_id="inv-1",
        idempotency_key="credit-meta",
    ...
    payment_event = next(
        row for row in rows
        if canonical_business_event_contract(row)["event_type"] == "invoice.payment_recorded"
    )
...
def test_invoice_void_and_uncollectible_replay_after_terminal_state() -> None:
    ...
    registry.create(
        business_id="business-a",
        invoice=_draft("inv-void"),
        idempotency_key="create-void",
        occurred_at_ms=100,
    )
    voided = registry.void(
        ...
        business_id="business-a",
        invoice_id="inv-void",
        idempotency_key="void-key",
        occurred_at_ms=200,
        event_metadata={"actor_id": "owner-1"},
    )
    ...
    registry.create(
        business_id="business-a",
        invoice=_draft("inv-uncollectible"),
        idempotency_key="create-uncollectible",
        occurred_at_ms=100,
    )
    ...
    registry.issue(
        tenant_id="tenant-a",
        business_id="business-a",
        invoice_id="inv-uncollectible",
        idempotency_key="issue-uncollectible",
        issued_at=issued_at,
        occurred_at_ms=200,
    )
    uncollectible = registry.mark_uncollectible(
        tenant_id="tenant-a",
        business_id="business-a",
        invoice_id="inv-uncollectible",
        idempotency_key="uncollectible-key",
        occurred_at_ms=300,
        event_metadata={"actor_id": "owner-1"},
    )
    assert registry.mark_uncollectible(
        tenant_id="tenant-a",
        business_id="business-a",
        invoice_id="inv-uncollectible",
        idempotency_key="uncollectible-key",
