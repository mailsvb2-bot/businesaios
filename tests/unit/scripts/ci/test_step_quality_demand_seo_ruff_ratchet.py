...
def test_demand_seo_i001_ratchet_blocks_regressions(
    tmp_path: Path,
    ...
    outcomes = iter([_outcome(returncode=0)] * 14 + [_outcome(returncode=1)])
    ...
    monkeypatch.setattr(
        quality,
        "_full_debt_report",
        lambda **_kwargs: {"full_ruff_measured": True, "full_ruff_total": 4853},
    )

    ok, message, payload = quality._ruff_check()

    assert ok is False
    assert message == "demand_seo I001 ruff ratchet failed"
    assert payload["demand_seo_i001_passed"] is False
    assert payload["violations"] == ["demand_seo_i001_ratchet_failed"]
