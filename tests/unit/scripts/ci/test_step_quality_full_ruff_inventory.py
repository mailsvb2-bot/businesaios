...
def test_headless_import_ratchet_blocks_regressions(
    ...
    monkeypatch.setattr(quality, "_quality_target_paths", lambda _root: (tmp_path / "runtime",))
    monkeypatch.setattr(quality.importlib.util, "find_spec", lambda _name: object())
    outcomes = iter((_outcome(returncode=0), _outcome(returncode=0), _outcome(returncode=1)))
    monkeypatch.setattr(quality, "run_command", lambda *_args, **_kwargs: next(outcomes))
    monkeypatch.setattr(quality, "_targeted_debt_report", lambda **_kwargs: {"targeted_strict_debt_measured": True, "targeted_strict_debt_total": 0})
    monkeypatch.setattr(quality, "_full_debt_report", lambda **_kwargs: {"full_ruff_measured": True, "full_ruff_total": 4875})

    ok, message, payload = quality._ruff_check()

    assert ok is False
    assert message == "headless I001,UP035 ruff ratchet failed"
    assert payload["headless_i001,up035_passed"] is False
...
def test_leads_i001_ratchet_blocks_regressions(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(quality, "repo_root", lambda: tmp_path)
    ...
    assert ok is False
    assert message == "leads I001 ruff ratchet failed"
    assert payload["leads_i001_passed"] is False
    assert payload["violations"] == ["leads_i001_ratchet_failed"]


def test_mvp_i001_ratchet_blocks_regressions(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(quality, "repo_root", lambda: tmp_path)
    ...
    monkeypatch.setattr(quality, "run_command", lambda *_args, **_kwargs: next(outcomes))
    monkeypatch.setattr(quality, "_targeted_debt_report", lambda **_kwargs: {"targeted_strict_debt_measured": True, "targeted_strict_debt_total": 0})
    monkeypatch.setattr(quality, "_full_debt_report", lambda **_kwargs: {"full_ruff_measured": True, "full_ruff_total": 4866})

    ok, message, payload = quality._ruff_check()

    assert ok is False
    assert message == "mvp I001 ruff ratchet failed"
    assert payload["mvp_i001_passed"] is False
    assert payload["violations"] == ["mvp_i001_ratchet_failed"]


def test_demand_decision_i001_ratchet_blocks_regressions(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(quality, "repo_root", lambda: tmp_path)
    ...
    monkeypatch.setattr(quality, "run_command", lambda *_args, **_kwargs: next(outcomes))
    monkeypatch.setattr(quality, "_targeted_debt_report", lambda **_kwargs: {"targeted_strict_debt_measured": True, "targeted_strict_debt_total": 0})
    monkeypatch.setattr(quality, "_full_debt_report", lambda **_kwargs: {"full_ruff_measured": True, "full_ruff_total": 4864})

    ok, message, payload = quality._ruff_check()

    assert ok is False
    assert message == "demand_decision I001 ruff ratchet failed"
    assert payload["demand_decision_i001_passed"] is False
    assert payload["violations"] == ["demand_decision_i001_ratchet_failed"]
