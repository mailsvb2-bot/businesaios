...
def test_presentation_i001_ratchet_blocks_regressions(
    tmp_path: Path,
    ...
    outcomes = iter([_outcome(returncode=0)] * 12 + [_outcome(returncode=1)])
    ...
    monkeypatch.setattr(
        quality,
        "_full_debt_report",
        lambda **_kwargs: {"full_ruff_measured": True, "full_ruff_total": 4860},
    )

    ok, message, payload = quality._ruff_check()

    assert ok is False
    assert message == "presentation I001 ruff ratchet failed"
    assert payload["presentation_i001_passed"] is False
    assert payload["violations"] == ["presentation_i001_ratchet_failed"]
