...
def test_routing_i001_ratchet_blocks_regressions(
    tmp_path: Path,
    ...
    def run_command(args, **_kwargs):
        is_target = str(tmp_path / "routing") in args and "--select" in args and args[args.index("--select") + 1] == "I001"
    ...
    monkeypatch.setattr(
        quality,
        "_full_debt_report",
        lambda **_kwargs: {"full_ruff_measured": True, "full_ruff_total": 4845},
    )

    ok, message, payload = quality._ruff_check()

    assert ok is False
    assert message == "routing I001 ruff ratchet failed"
    assert payload["routing_i001_passed"] is False
    assert payload["violations"] == ["routing_i001_ratchet_failed"]
