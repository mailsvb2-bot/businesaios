...
def test_ports_e402_ratchet_blocks_regressions(
    tmp_path: Path,
    ...
    outcomes = iter([_outcome(returncode=0)] * 11 + [_outcome(returncode=1)])
    ...
    monkeypatch.setattr(
        quality,
        "_full_debt_report",
        lambda **_kwargs: {"full_ruff_measured": True, "full_ruff_total": 4862},
    )

    ok, message, payload = quality._ruff_check()

    assert ok is False
    assert message == "ports E402 ruff ratchet failed"
    assert payload["ports_e402_passed"] is False
    assert payload["violations"] == ["ports_e402_ratchet_failed"]
