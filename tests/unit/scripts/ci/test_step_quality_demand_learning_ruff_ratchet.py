...
def test_demand_learning_i001_ratchet_blocks_regressions(
    tmp_path: Path,
    ...
    outcomes = iter([_outcome(returncode=0)] * 13 + [_outcome(returncode=1)])
    ...
    monkeypatch.setattr(
        quality,
        "_full_debt_report",
        lambda **_kwargs: {"full_ruff_measured": True, "full_ruff_total": 4857},
    )

    ok, message, payload = quality._ruff_check()

    assert ok is False
    assert message == "demand_learning I001 ruff ratchet failed"
    assert payload["demand_learning_i001_passed"] is False
    assert payload["violations"] == ["demand_learning_i001_ratchet_failed"]
