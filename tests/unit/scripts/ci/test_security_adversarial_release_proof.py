...
def test_verify_release_artifact_aggregates_security_without_second_verdict(monkeypatch, tmp_path) -> None:
    ...
    ok, _ = step_verify_release._aggregate_required_proof_artifacts(security_evidence=security)
    payload = json.loads((artifact_dir / "verify_release.json").read_text(encoding="utf-8"))

    assert ok is True
    assert payload["status"] == "ready"
    assert payload["exact_sha"] == "c" * 40
    assert payload["security_adversarial"] == security
    assert payload["claims_production_ready"] is False

    blocked = dict(security, status="FAIL", violations=["security_junit_skipped"])
    ok, _ = step_verify_release._aggregate_required_proof_artifacts(security_evidence=blocked)
    payload = json.loads((artifact_dir / "verify_release.json").read_text(encoding="utf-8"))
    assert ok is False
    assert payload["status"] == "blocked"
    assert "security_junit_skipped" in payload["violations"]
