...
def test_contracts_up012_ratchet_blocks_regressions(
    tmp_path: Path,
    ...
    outcomes = iter([_outcome(returncode=0)] * 9 + [_outcome(returncode=1)])
    ...
    monkeypatch.setattr(
        quality,
        "_full_debt_report",
        lambda **_kwargs: {"full_ruff_measured": True, "full_ruff_total": 4864},
    )

    ok, message, payload = quality._ruff_check()

    assert ok is False
    assert message == "contracts UP012 ruff ratchet failed"
    assert payload["contracts_up012_passed"] is False
    assert payload["violations"] == ["contracts_up012_ratchet_failed"]
