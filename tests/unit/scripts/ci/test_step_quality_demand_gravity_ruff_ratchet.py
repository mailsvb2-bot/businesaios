...
def test_demand_gravity_f401_ratchet_blocks_regressions(
    tmp_path: Path,
    ...
    outcomes = iter([_outcome(returncode=0)] * 10 + [_outcome(returncode=1)])
    ...
    monkeypatch.setattr(
        quality,
        "_full_debt_report",
        lambda **_kwargs: {"full_ruff_measured": True, "full_ruff_total": 4863},
    )

    ok, message, payload = quality._ruff_check()

    assert ok is False
    assert message == "demand_gravity F401 ruff ratchet failed"
    assert payload["demand_gravity_f401_passed"] is False
    assert payload["violations"] == ["demand_gravity_f401_ratchet_failed"]
