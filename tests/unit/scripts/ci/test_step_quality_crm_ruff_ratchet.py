...
def test_crm_f401_ratchet_blocks_regressions(
    tmp_path: Path,
    ...
    outcomes = iter([_outcome(returncode=0)] * 8 + [_outcome(returncode=1)])
    ...
    monkeypatch.setattr(
        quality,
        "_full_debt_report",
        lambda **_kwargs: {"full_ruff_measured": True, "full_ruff_total": 4865},
    )

    ok, message, payload = quality._ruff_check()

    assert ok is False
    assert message == "crm F401 ruff ratchet failed"
    assert payload["crm_f401_passed"] is False
    assert payload["violations"] == ["crm_f401_ratchet_failed"]
