...
def test_opportunity_explainer_redacts_nested_sensitive_payload() -> None:
    candidate = DecisionCandidate(
        ...
        expected_value=0.8,
        confidence=0.9,
        payload={'profile': {'customer_email': 'a@example.com'}, 'business_id': 'biz-1'},
    )
    payload = OpportunityExplainer().explain(candidate)['payload']
    assert payload['profile']['customer_email'] == '<redacted>'
