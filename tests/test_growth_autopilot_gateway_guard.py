...
def test_growth_autopilot_requires_gateway_and_decision_route():
    sink = _Sink()
    gw = _Gateway()
    ...
    assert all(call["payload"]["decision_id"] == "d1" for call in gw.calls)


def test_growth_autopilot_blocks_direct_apply_service():
    sink = _Sink()
    gw = _Gateway()
    engine = AutopilotEngine(
        entitlements_provider=_Ents(),
    ...
    assert "direct apply is forbidden" in out.message
