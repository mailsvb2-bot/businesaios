...
from scripts.ci.regression_impact import blocked_artifact_paths, required_fast_steps_for_paths
...
def test_baseline_single_decision_flow_contract_is_declared() -> None:
    ...
    assert "single DecisionCore" in requirement.title
    assert {"fast", "full", "release", "pre-release"}.issubset(requirement.required_gates)
...
def test_baseline_generated_artifact_contract_is_blocked() -> None:
    ...
    assert blocked_artifact_paths(("runtime/data/security/events.jsonl",)) == (
        "runtime/data/security/events.jsonl",
    )
