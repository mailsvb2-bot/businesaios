...
def _within_window(
    *,
    ...
    return (
        ...
        and delta <= timedelta(days=max(1, int(days)))
    )
...
class ClientAttributionPolicy:
    ...
    def evaluate(
        ...
        *,
        ...
        lead: OutcomeLead,
        proofs: Iterable[ClientProofEvent],
    ) -> AttributionVerdict:
        items = tuple(proofs)
        if self.require_tracking_token and not lead.tracking_token:
            return AttributionVerdict(
                ...
                attributed=False,
                reason_code="missing_tracking_token",
        ...
        for proof in items:
            if (
                proof.lead_id != lead.lead_id
                or proof.business_id != lead.business_id
            ):
                continue
            if not _within_window(
                captured_at=lead.captured_at,
                occurred_at=proof.occurred_at,
                days=order.package.attribution_window_days,
            ):
                continue
        ...
        if (
            self.require_identity_or_external_ref
            and not identity_present
            and not accepted_refs
        ):
            return AttributionVerdict(
                lead_id=lead.lead_id,
                attributed=False,
                reason_code="missing_identity_and_external_ref",
                source_of_truth="client_attribution_policy",
                confidence=0.15,
            )
