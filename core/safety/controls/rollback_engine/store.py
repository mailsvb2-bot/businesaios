...
from .models import RollbackAction, RollbackExecutionState, RollbackPlan, RollbackReceipt, RollbackReconciliationState
...
class SqliteRollbackPlanStore:
    ...
    def _init_db(self) -> None:
        plan = SchemaMigrationPlan(component='rollback_plans', target_version=SCHEMA_VERSION, steps=(self._migrate_v1, self._migrate_v2, self._migrate_v3, self._migrate_v4, self._migrate_v5))
    ...
    def put(self, *, tenant_id: str, action_id: str, plan: RollbackPlan) -> RollbackPlan:
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                ''',
                (tenant_key, action_key, json.dumps(_serialize_plan(plan), sort_keys=True), str(plan.execution_state.value if hasattr(plan.execution_state,'value') else plan.execution_state), str(plan.confirmation_token or ''), json.dumps([asdict(item) for item in plan.receipts], sort_keys=True), str(plan.reconciliation_state.value if hasattr(plan.reconciliation_state,'value') else plan.reconciliation_state), str(plan.reconciliation_error or ''), next_version, str(plan.lease_owner or ''), int(plan.fencing_token or 0)),
            )
    ...
    def get(self, *, tenant_id: str, action_id: str) -> RollbackPlan | None:
        ...
        if not action_key:
            raise ValueError('action_id is required')
        with self._connect() as conn:
            row = conn.execute('SELECT plan_json, execution_state, confirmation_token, receipts_json, reconciliation_state, reconciliation_error, version, lease_owner, fencing_token FROM safety_rollback_plans WHERE tenant_id = ? AND action_id = ?', (tenant_key, action_key)).fetchone()
        if row is None:
            return None
        return _deserialize_plan(json.loads(str(row[0]) or '{}'), execution_state=str(row[1] or 'planned'), confirmation_token=str(row[2] or ''), receipts=json.loads(str(row[3]) or '[]'), reconciliation_state=str(row[4] or 'pending'), reconciliation_error=str(row[5] or ''), version=int(row[6] or 0), lease_owner=str(row[7] or ''), fencing_token=int(row[8] or 0))
    ...
    def compare_and_set(self, *, tenant_id: str, action_id: str, expected_version: int, plan: RollbackPlan) -> RollbackPlan:
        ...
        with self._connect() as conn:
            updated = conn.execute(
                ...
                ''',
                (json.dumps(_serialize_plan(plan), sort_keys=True), str(plan.execution_state.value if hasattr(plan.execution_state,'value') else plan.execution_state), str(plan.confirmation_token or ''), json.dumps([asdict(item) for item in plan.receipts], sort_keys=True), str(plan.reconciliation_state.value if hasattr(plan.reconciliation_state,'value') else plan.reconciliation_state), str(plan.reconciliation_error or ''), next_version, str(plan.lease_owner or ''), next_fencing_token, tenant_key, action_key, int(expected_version)),
            ).rowcount
            if updated == 0:
                raise RuntimeError('rollback_plan_version_conflict')
    ...
    def acquire_lease(self, *, tenant_id: str, action_id: str, owner: str) -> RollbackPlan | None:
        ...
        if current is None:
            return None
        return self.compare_and_set(tenant_id=tenant_id, action_id=action_id, expected_version=int(current.version), plan=RollbackPlan(**{**current.__dict__, 'lease_owner': str(owner), 'fencing_token': int(current.fencing_token) + 1}))
...
def _deserialize_plan(data: dict[str, object], *, execution_state: str, confirmation_token: str, receipts: list[dict[str, object]], reconciliation_state: str, reconciliation_error: str, version: int = 0, lease_owner: str = '', fencing_token: int = 0) -> RollbackPlan:
    return RollbackPlan(
        ...
        receipts=tuple(RollbackReceipt(step_index=int(item.get('step_index') or 0), action=str(item.get('action') or ''), status=str(item.get('status') or ''), details=dict(item.get('details') or {})) for item in list(receipts or [])),
...
__all__ = ['CANON_SAFETY_ROLLBACK_STORE', 'RollbackPlanStore', 'InMemoryRollbackPlanStore', 'SqliteRollbackPlanStore']
