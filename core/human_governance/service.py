...
class HumanGovernanceService:
    """Thin review/override orchestration; DecisionCore remains the sole decision issuer."""
    ...
    def __init__(
        ...
        self._unauthorized_override_guard = unauthorized_override_guard or UnauthorizedOverrideGuard()
        self._review_case_builder = ReviewCaseBuilder(approval_state_reader=deps.approval_state_reader, policy=deps.policy)
        self._override_case_builder = OverrideCaseBuilder(override_repository=deps.override_repository)
        self._escalation_case_builder = EscalationCaseBuilder(escalation_reader=deps.escalation_reader)

    def build_review_case(self, review_id: str) -> ReviewCase:
        return self._review_case_builder.build(self._require_review(review_id))
    ...
    def evaluate_sales_handoff(self, **facts: object) -> dict[str, object] | None:
        return self._policy.evaluate_sales_handoff(**facts)
    ...
    def override(self, record: OverrideRecord, allowed_actor_ids: set[str]) -> OverrideRecord:
        ...
        self._policy.ensure_actionable(review.status)
        self._unauthorized_override_guard.ensure_allowed(actor_id=record.actor_id, allowed_actor_ids=allowed_actor_ids)
    ...
    def pause(self, review_id: str, reason: str, actor_id: str) -> ApprovalState:
        ...
        self._policy.ensure_actionable(review.status)
        return self._deps.pause_writer.write_pause(review_id=review_id, reason=normalized_reason, actor_id=normalized_actor_id)
