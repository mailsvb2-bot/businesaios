...
def decide_for_day(
    ...
    *,
    ...
    fx_mod.store_features_for_day(store, tenant_id=tenant_id, user_id=user_id, day_key=day_key, features=features, now_ms=now_ms)
    ...
    reason = "suppressed"
    if not suppressed and has_active_entitlement(store, tenant_id=tenant_id, user_id=user_id, now_ms=now_ms, entitlements=entitlements):
    ...
    if not suppressed and daily_offer_cap_reached(store, tenant_id=tenant_id, user_id=user_id, day_key=day_key, now_ms=now_ms, policy=policy):
        suppressed, reason = True, "daily_cap"
    debug: dict[str, Any] = {"features": dict(features), "hazard": hazard, "readiness": readiness, "suppressed": suppressed, "now_ms": now_ms}
    if suppressed:
        return _day_decision(
            tenant_id=tenant_id,
            day_key=day_key,
            day_index=day_index,
            hazard=hazard,
    ...
    candidates = filter_candidate_arms(store, tenant_id=tenant_id, user_id=user_id, candidates=candidates, now_ms=now_ms, debug=debug, logger=logger)
    if not candidates:
        return _day_decision(
            tenant_id=tenant_id,
            day_key=day_key,
    ...
    offer_arm = choose_arm_event_sourced(store, tenant_id=tenant_id, user_id=user_id, arms=candidates, now_ms=now_ms)
...
class RetentionEngine:
    ...
    def compute_decision(
        ...
        *,
        ...
        if not is_retention_allowed(tenant_id=self._tenant_id, user_id=str(user_id), fallback_allow=True):
            return RetentionDayDecision(**build_sandbox_suppressed_decision(tenant_id=self._tenant_id, day_key=str(day_key), day_index=int(day_index), policy=self._policy))
    ...
    def decide_offer(self, *, tenant_id: str, user_id: str, context: dict) -> RetentionDecision | None:
        ...
        if requested_tenant != self._tenant_id or not is_retention_allowed(tenant_id=self._tenant_id, user_id=str(user_id), fallback_allow=False):
            return None
        ...
        return RetentionDecision(
            ...
            score=float(decision.readiness) * (float(self._policy.score_complement_base) - float(decision.hazard)),
        )
