...
def main(argv:
    ...
    if args.command=='rollback-recommendation':
        print(json.dumps(governance.rollback_recommendation(baseline_name=args.baseline_name, candidate_run_id=args.candidate_run_id, fallback_run_ids=args.fallback_run_id), ensure_ascii=False, indent=2, sort_keys=True))
        return 0
    if args.command=='joined-history':
        print(json.dumps(governance.joined_history(baseline_name=args.baseline_name, candidate_run_ids=args.candidate_run_id), ensure_ascii=False, indent=2, sort_keys=True))
        return 0
    if args.command=='verify-evidence':
        print(json.dumps(governance.verify_promotion_evidence(baseline_name=args.baseline_name), ensure_ascii=False, indent=2, sort_keys=True))
        return 0
    ...
    if args.command=='trend':
        print(json.dumps(governance.drift_trend(baseline_name=args.baseline_name, candidate_run_ids=list(args.candidate_run_id)), ensure_ascii=False, indent=2, sort_keys=True))
        return 0
    if args.command=='memory-summary':
        print(json.dumps(governance.memory_summary(tenant_id=args.tenant_id, business_id=args.business_id), ensure_ascii=False, indent=2, sort_keys=True))
        return 0
    return 2
