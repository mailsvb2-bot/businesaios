...
def register_fastapi_routes(*, app: Any, bundle) -> None:
    @app.get('/api/debug/messaging-policy-traces')
    def debug_messaging_policy_traces_json(tenant_id: str = 'default', user_id: str = '', date_from: str = '', date_to: str = '', limit: int = 50):
        res = bundle.json(tenant_id=tenant_id, user_id=user_id, date_from=date_from, date_to=date_to, limit=limit)
        return res.body

    @app.get('/debug/messaging-policy-traces')
    def debug_messaging_policy_traces_html(tenant_id: str = 'default', user_id: str = '', date_from: str = '', date_to: str = '', limit: int = 50):
