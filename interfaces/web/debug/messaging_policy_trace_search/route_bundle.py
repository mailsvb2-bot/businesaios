...
class MessagingPolicyTraceSearchRouteBundle:
    ...
    def json(self, *, tenant_id, user_id, date_from, date_to, limit):
        query = parse_trace_search_query(tenant_id=clean_text(tenant_id, default='default'), user_id=clean_text(user_id), date_from=clean_text(date_from), date_to=clean_text(date_to), limit=limit)
