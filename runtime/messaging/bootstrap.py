...
class _NativeProviderQueueAdapter:
    ...
    def send(self, msg) -> DeliveryResult:
        ...
        payload = ProviderPayloadNormalizers().normalize_outbound(provider=service.provider_registry.get(self.provider_key), operation="message_send", payload={"user_id": msg.user_id, "text": msg.text, **{key: context[key] for key in ("peer_id", "chat_id", "random_id", "channel_id", "recipient_id", "to", "receiver") if key in context}})
