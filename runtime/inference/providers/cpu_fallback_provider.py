...
CANON_RUNTIME_CPU_FALLBACK_PROVIDER = True


class CPUFallbackProvider:
    name = 'cpu_fallback_provider'

    def __init__(self) -> None:
        self.profile = InferenceCapacityProfile(
            tier=InferenceCapacityTier.CPU_FALLBACK,
            limits=InferenceCapacityLimits(2, 8192, 2048, 8),
            estimated_cost_per_1k_tokens_usd=0.0025,
            description='Deterministic CPU fallback under runtime control.',
        )
        self._state = BaseInferenceProviderState(latency_ms=1800, saturation_score=0.30)
    ...
    def infer(self, request: InferenceRequest) -> InferenceResponse:
        output = f"[cpu-fallback] {request.prompt[:256]}"
