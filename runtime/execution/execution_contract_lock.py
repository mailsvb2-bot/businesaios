...
It owns no decision logic and no effect dispatching. External effects require
independently attributed evidence before outcome persistence. Hermetic no-op
transports are treated separately because they prove that no external actuation
occurred and therefore cannot create an external-evidence obligation.
"""
...
class ExecutionContractLockError(RuntimeError):
    """Raised when post-dispatch verification cannot authorize persistence."""
...
def _conditional_confirmation_mode(
    *,
    ...
    if action_name == "suggest_offer_patch@v1":
        return "required" if str(payload.get("notify_user_id") or "").strip() else "not_required"
    ...
    if action_name == "apply_offer_patch@v1":
        ...
        return "not_required" if outcome in _NON_ACTUATION_OUTCOMES else "required"
...
def _action_verification_contract(
    ...
    # A hermetic transport no-op means no external mutation occurred. Its local
    # receipt is sufficient to prove the deterministic no-op outcome and must
    # not be confused with provider confirmation for a real external effect.
    if offline_effect_noop(normalized_output):
        return category, "not_required"

    requested_mode = str(
        payload.get("external_confirmation_mode")
        or payload.get("confirmation_mode")
        or ""
    ).strip().casefold()
...
def commit_verified_execution(
    *,
    executor: Any,
    env: Any,
    output: Mapping[str, Any] | None,
    verification_result: VerificationStageResult,
) -> dict[str, Any]:
    if not verification_result.verified:
        raise ExecutionContractLockError("unverified_commit_forbidden")
    normalized_output = _safe_dict(output)
