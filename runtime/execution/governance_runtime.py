...
def _apply_approval_workflow_resolution(*, workflow: ApprovalWorkflow, approval_decision: ApprovalDecision) -> Any:
    return workflow.resolve(approval_decision)


def _gate_metadata(
    *, payload: dict[str, Any], meta: dict[str, Any], env: Any, impact: ActionImpact
) -> dict[str, object]:
    ...
    return {
        ...
        "actor_id": str(
            meta.get("actor_id") or payload.get("actor_id") or payload.get("user_id") or meta.get("user_id") or "system"
        ).strip(),
...
def _build_approval_output(*, verdict: Any) -> dict[str, object]:
    ...
    payload: dict[str, object] = {
        ...
        "manual_override_used": bool(
            metadata.get("manual_override_used", getattr(verdict, "used_operator_override", False))
        ),
        "manual_override_allowed": bool(
            _safe_dict(getattr(verdict, "policy", {})).get("manual_override_allowed", False)
        ),
    ...
    return {
        ...
        or key in {"approval_required", "operator_required", "manual_override_used", "manual_override_allowed"}
    }


def _build_resume_governance_hint(
    *, ctx: Any, approval_id: str | None, gate_verdict: Any | None, operator_override_id: str | None
) -> dict[str, object]:
    hint = {
...
def review_governance_execution(*, executor: Any, env: Any) -> None:
    ...
    if operator_override_id:
        ...
        if operator_override is None:
            resume = _build_resume_governance_hint(
                ctx=ctx, approval_id=approval_id, gate_verdict=None, operator_override_id=operator_override_id
            )
            _emit_resume_event(
                executor=executor,
                guard=guard,
    ...
    if _approval_gate_enabled(payload=payload, meta=meta, impact=impact):
        ...
        gate_verdict = gate.evaluate(
            ...
            external_confirmation_mode=str(
                payload.get("external_confirmation_mode") or meta.get("external_confirmation_mode") or "required"
            ),
        ...
        if bool(getattr(gate_verdict, "used_operator_override", False)):
            if operator_override is None:
                raise GovernanceExecutionBlocked(
                    ...
                    output={
                        ...
                        "operator_required": True,
                        "governance": {
                            "reason": "operator_override_consumption_requires_record",
                            "resume": _build_resume_governance_hint(
                                ctx=ctx,
                                approval_id=approval_id,
                                gate_verdict=gate_verdict,
            ...
            _emit_resume_event(
                ...
                resume=_build_resume_governance_hint(
                    ...
                    approval_id=approval_id,
                    gate_verdict=gate_verdict,
                    operator_override_id=consumed_override.request.override_id,
                ),
    ...
    if not verdict.allowed:
        resume = _build_resume_governance_hint(
            ctx=ctx, approval_id=approval_id, gate_verdict=gate_verdict, operator_override_id=operator_override_id
        )
        _emit_resume_event(
            executor=executor,
            guard=guard,
        ...
        if getattr(executor, "_events", None) is not None:
            executor._events.emit(
                ...
                payload={
                    ...
                    "verdict": {
                        "reason": verdict.reason,
                        "required_permission": verdict.required_permission.value
                        if verdict.required_permission
                        else None,
...
def _build_impact(*, ctx: Any, payload: dict[str, Any], meta: dict[str, Any]) -> ActionImpact:
    ...
    impact = ActionImpact(
        ...
        strategic_change_count=_normalize_non_negative_int(
            payload.get("strategic_change_count"), meta.get("strategic_change_count")
        ),
        rollback_event_count=_normalize_non_negative_int(
            payload.get("rollback_event_count"), meta.get("rollback_event_count")
        ),
