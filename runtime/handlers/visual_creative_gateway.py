...
def handle_generate_visual_creative(payload: dict[str, Any], effects: EffectsPort, env: Any) -> Any:
    ...
    return effects.generate_visual_creative(
        ...
        kind=required_str(body, "kind"),
        prompt=required_str(body, "prompt"),
        country_code=optional_str(body, "country_code") or "",
        preferred_provider=optional_str(body, "preferred_provider") or "",
        aspect_ratio=optional_str(body, "aspect_ratio") or "1:1",
        duration_seconds=int(body.get("duration_seconds") or 5),
        negative_prompt=optional_str(body, "negative_prompt") or "",
        reference_url=optional_str(body, "reference_url") or "",
        brand_context=optional_str(body, "brand_context") or "",
        wait_seconds=int(body.get("wait_seconds") or 0),
    )


def handle_poll_visual_creative(payload: dict[str, Any], effects: EffectsPort, env: Any) -> Any:
    body = require_mapping(payload)
    return effects.poll_visual_creative(
        decision_id=str(env.decision.decision_id),
        correlation_id=str(env.decision.correlation_id),
        tenant_id=required_str(body, "tenant_id"),
        user_id=required_str(body, "user_id"),
