...
def build_messaging_policy_alert_subscription_service(*, alert_service, settings_gateway=None, tenant_id: str = '', cooldown_s: int = 3600):
    dedup_stack = build_deduping_alert_notifier(cooldown_s=int(cooldown_s)) if settings_gateway is None else build_tenant_aware_persistent_deduping_alert_notifier(settings_gateway=settings_gateway, cooldown_s=int(cooldown_s), tenant_id=tenant_id)
    return {'service': MessagingPolicyAlertSubscriptionService(alert_service=alert_service, notifier=dedup_stack['notifier']), 'notifier_stack': dedup_stack}
