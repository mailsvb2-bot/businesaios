...
def _build_send_kwargs(payload: dict, env) -> dict:
    ...
    if channel in {"vk", "max", "slack", "discord", "instagram", "messenger"}:
        track_payload["_provider_native"] = {"provider_key": f"{channel}_messaging", **{key: payload.get(key) for key in ("business_id", "approval_id", "peer_id", "chat_id", "random_id", "channel_id", "recipient_id") if payload.get(key) not in {None, ""}}}
    return {
