...
_DDL_EVENT_COUNTERS = (
    ...
    "event_type TEXT NOT NULL, user_id TEXT NOT NULL, cnt INTEGER NOT NULL, "
    "last_ts_ms INTEGER NOT NULL, PRIMARY KEY (event_type, user_id))"
)
_DDL_USER_STATE = (
    "CREATE TABLE IF NOT EXISTS user_state (tenant_id TEXT NOT NULL, "
    "user_id TEXT NOT NULL, state_json TEXT NOT NULL, updated_at_ms INTEGER NOT NULL, "
    "PRIMARY KEY (tenant_id, user_id))"
)
_DDL_USER_FEATURES_DAILY = (
    "CREATE TABLE IF NOT EXISTS user_features_daily (tenant_id TEXT NOT NULL, "
    "user_id TEXT NOT NULL, day_key TEXT NOT NULL, features_json TEXT NOT NULL, "
    "created_at_ms INTEGER NOT NULL, PRIMARY KEY (tenant_id, user_id, day_key))"
)
_DDL_BANDIT_ARMS = (
    "CREATE TABLE IF NOT EXISTS bandit_arms (tenant_id TEXT NOT NULL, arm TEXT NOT NULL, "
    "alpha REAL NOT NULL DEFAULT 1.0, beta REAL NOT NULL DEFAULT 1.0, "
    "last_updated_at_ms INTEGER NOT NULL, PRIMARY KEY (tenant_id, arm))"
)
_DDL_JOB_LOCKS = (
    "CREATE TABLE IF NOT EXISTS job_locks (tenant_id TEXT NOT NULL, job_key TEXT NOT NULL, "
    "locked_at_ms INTEGER NOT NULL, PRIMARY KEY (tenant_id, job_key))"
)
_DDL_SETTINGS = (
    "CREATE TABLE IF NOT EXISTS settings (tenant_id TEXT NOT NULL, key TEXT NOT NULL, "
    "value_json TEXT NOT NULL, updated_at_ms INTEGER NOT NULL, PRIMARY KEY (tenant_id, key))"
)


def init_schema(db: sqlite3.Connection) -> None:
    """Create all tables, migrations, indexes, and legacy-write cursor bridge."""
    db.execute(_DDL_EVENTS)
    _maybe_add_tenant_id_column(db)
    _maybe_add_append_seq_column(db)
    db.execute(_DDL_EVENT_APPEND_SEQUENCE)
    db.execute(
        "INSERT OR IGNORE INTO event_append_sequence(singleton,last_seq) VALUES (1,0)"
    )
    _backfill_append_sequences(db)
    _install_legacy_append_sequence_trigger(db)
    db.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_events_append_seq ON events(append_seq)")
    db.execute("CREATE INDEX IF NOT EXISTS idx_events_tenant_append_seq ON events(tenant_id,append_seq)")
    db.execute("CREATE INDEX IF NOT EXISTS idx_events_tenant_decision_type ON events(tenant_id,decision_id,event_type)")
...
def _event_columns(db: sqlite3.Connection) -> set[str]:
    return {str(row[1]).lower() for row in db.execute("PRAGMA table_info(events)").fetchall()}
...
def _backfill_append_sequences(db: sqlite3.Connection) -> None:
    row = db.execute("SELECT last_seq FROM event_append_sequence WHERE singleton=1").fetchone()
    ...
    for event_row in db.execute(
        ...
        current += 1
        db.execute("UPDATE events SET append_seq=? WHERE event_id=?", (current, str(event_row[0])))
    db.execute("UPDATE event_append_sequence SET last_seq=? WHERE singleton=1", (current,))


def _install_legacy_append_sequence_trigger(db: sqlite3.Connection) -> None:
    """Assign durable cursors to direct/legacy INSERTs that omit append_seq."""
    db.execute("DROP TRIGGER IF EXISTS trg_events_assign_append_seq")
    db.execute(
        """
        CREATE TRIGGER trg_events_assign_append_seq
        AFTER INSERT ON events
        WHEN NEW.append_seq IS NULL
        BEGIN
          UPDATE event_append_sequence
