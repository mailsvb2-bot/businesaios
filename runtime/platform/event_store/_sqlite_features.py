"""SQLite retention-SaaS helpers: feature store, bandit arms, job locks.

Pure functions taking an open sqlite3.Connection.
"""
...
def bandit_ensure_arm(db, *, tenant_id: str, arm: str, now_ms: int) -> None:
    db.execute(
        "INSERT OR IGNORE INTO bandit_arms(tenant_id,arm,alpha,beta,last_updated_at_ms) "
        "VALUES(?,?,1.0,1.0,?)",
...
def bandit_get_arm(db, *, tenant_id: str, arm: str) -> tuple[float, float]:
    row = db.execute(
...
def bandit_update_arm(db, *, tenant_id: str, arm: str, success: bool, now_ms: int) -> None:
...
def try_lock_job(db, *, tenant_id: str, job_key: str, now_ms: int) -> bool:
    try:
        db.execute(
