...
class DemandSignalCandidateProducer:
    def build_candidates(
        ...
        *,
        ...
        for channel in sorted(grouped, key=lambda item: item.value):
            ...
            candidate_id = self._candidate_id(tenant_id=tenant_id, channel=channel, signals=channel_signals)
            payload = {
            ...
            candidate = DemandCandidate(
                ...
                created_at=now,
                payload=payload,
                idempotency_key=f"demand-gravity:{candidate_id}",
    ...
    @staticmethod
    def _candidate_id(*, tenant_id: str, channel: DemandChannel, signals: tuple[DemandSignal, ...]) -> str:
        raw = "|".join([tenant_id, channel.value, *sorted(s.raw_fingerprint for s in signals)]).encode()
