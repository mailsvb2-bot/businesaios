...
def parse_subscription(value: dict | None, *, tenant_id: str) -> AlertSubscriptionRecord | None:
    ...
    return AlertSubscriptionRecord(tenant_id=tenant_scope, recipient_user_id=recipient_user_id, channel=str(value.get("channel") or "telegram"), min_level=str(value.get("min_level") or "warn"), business_id=str(value.get("business_id") or ""), enabled=bool(value.get("enabled", True)), code_filters=tuple(value.get("code_filters") or ()), user_scope=tuple(value.get("user_scope") or ()))
