...
def build_tenant_aware_persistent_deduping_alert_notifier(*, settings_gateway, cooldown_s: int = 3600, tenant_id: str = ''):
    ...
    suppression_service = TenantAwareAlertNotificationSuppressionService(store_factory=store_factory, cooldown_s=int(cooldown_s), tenant_id=tenant_id)
    ...
    return {'store': store_factory, 'store_factory': store_factory, 'suppression_service': suppression_service, 'mark_sent_service': mark_sent_service, 'notifier': TenantAwareDedupingMessagingPolicyAlertNotifier(base_notifier=base_notifier, suppression_service=suppression_service, mark_sent_service=mark_sent_service)}
