...
def open_meteo_weather(city: str, *, transport: HttpTransport | None = None) -> Tuple[bool, str, Dict[str, object]]:
    try:
        ...
        if not isinstance(results, list) or not results: return False, "", {"error": "CITY_NOT_FOUND"}
        r0 = results[0] if isinstance(results[0], dict) else {}
        lat, lon = r0.get("latitude"), r0.get("longitude")
        if lat is None or lon is None: return False, "", {"error": "GEO_MISSING"}
        ...
        if not isinstance(cur, dict): return False, "", {"error": "WEATHER_MISSING"}
