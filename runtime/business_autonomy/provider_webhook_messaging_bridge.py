...
def resolve_provider_webhook_messaging_ingress(*, provider: ProviderDefinition, normalized_payload: Mapping[str, Any]) -> ProviderWebhookMessagingIngress | None:
    ...
    return ProviderWebhookMessagingIngress(channel=binding.channel, user_id=user_id, text=text, transport_message_id=transport_message_id, correlation_id=correlation_id)
