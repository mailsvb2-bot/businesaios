...
class ProviderLiveProbeRuntime:
    ...
    transports: Mapping[str, Any] = field(default_factory=dict)
    ...
    def __post_init__(self) -> None:
        if not self.transports:
            object.__setattr__(self, 'transports', build_provider_vendor_transports(self.secret_vault, bind_live_network=True))
    ...
    def run(self, *, provider: ProviderDefinition, tenant_id: str, business_id: str, mode: str = 'dry_run') -> ProviderLiveProbeResult:
        ...
        ok = bool(response.pop('_response_ok', http_status is None or 200 <= int(http_status) < 300))
        incident = None
        if not ok:
