...
class ProviderLiveSyncRuntime:
    ...
    def _finalize_result(self, *, tenant_id: str, business_id: str, provider: ProviderDefinition, operation: str, mode: str, result: ProviderSyncRunResult, payload: Mapping[str, Any]) -> ProviderSyncRunResult:
        refs = self.audit_recorder.record_sync_run(
            tenant_id=tenant_id, business_id=business_id, provider_key=provider.provider_key, operation=operation, mode=mode, status=result.status, accepted=result.accepted, payload=dict(payload or {}), metadata=dict(result.metadata)
        )
        ...
        history_row = self.sync_history.append({
            ...
            'recorded_at_utc': refs.get('recorded_at_utc') if isinstance(refs, dict) else None,
            'parsed_response': dict(result.metadata.get('parsed_response') or {}),
            'transport_response': dict(result.metadata.get('transport_response') or {}),
            'error': dict(result.metadata.get('error') or {}),
            'retry_policy': dict(result.metadata.get('retry_policy') or {}),
        })
        ...
        if not result.accepted or str(result.status).startswith('live_execution_failed'):
            ...
            incident = self.incident_registry.append({
                'tenant_id': str(tenant_id),
                'business_id': str(business_id),
                'provider_key': provider.provider_key,
                'kind': 'sync',
                'status': result.status,
                'severity': 'critical' if str(result.status) == 'live_execution_failed' else 'major',
                'category': str(error_view.get('category') or 'sync_runtime'),
                'message': str(error_view.get('message') or result.status),
                'retryable': bool(retry_view.get('retryable', False)),
                'metadata': {
                    'operation': operation,
                    'mode': mode,
                    'error': error_view,
                    'retry_policy': retry_view,
                },
            })
        else:
            incident = None
