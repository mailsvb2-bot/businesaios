...
This module must not call DecisionCore directly.
"""

log = logging.getLogger("runtime.telegram")


def maybe_autostart_yookassa_webhook(
    *,
    event_store,
    event_log,
    payment_outbox,
    ...
    try:
        ...
        wh_host = (
            env_str(prefix + "HOST", "127.0.0.1").strip()
            or "127.0.0.1"
        )
        wh_path = (
            env_str(prefix + "PATH", "/yookassa/webhook").strip()
            or "/yookassa/webhook"
        )
        wh_auth = (
            env_str(prefix + "AUTH_MODE", "token").strip().lower()
            or "token"
        )
...
def run_telegram(
    ...
    **_ignored,
    ...
    if not token:
        raise SystemExit(
            "RUN_MODE=telegram requires a Telegram bot token "
    ...
    maybe_autostart_yookassa_webhook(
        event_store=event_store,
        event_log=event_log,
        payment_outbox=payment_outbox,
    )
    ...
    def _state() -> dict:
        ...
        pending_outbox = None
        try:
            if hasattr(payment_outbox, "list_pending"):
                pending_outbox = len(payment_outbox.list_pending())
        except Exception:
            pending_outbox = None
    ...
    run_fn = getattr(runner, "run_forever", None) or getattr(
        runner,
        "run",
        None,
    )
...
__all__ = ['maybe_autostart_yookassa_webhook', 'run_telegram']
